...
def migrate():
    ...
    detail_tables = ['agoda_reviews', 'booking_reviews', 'google_reviews', 'tripadvisor_reviews', 'review_media']
    ...
    with engine.begin() as conn:
        for table in all_tables:
            ...
            if not check_col:
                ...
                sys.stdout.flush()
                conn.execute(text(f"ALTER TABLE {table} ADD review_uuid VARCHAR(36) NULL"))
            else:
    ...
    with engine.begin() as conn:
        ...
        for table in all_tables:
            ...
            for row in res:
                ...
                is_pk = conn.execute(text(f"SELECT is_primary_key FROM sys.indexes WHERE name = '{row.name}' AND object_id = OBJECT_ID('{table}')")).scalar()
                if is_pk:
        ...
        for table in all_tables:
            ...
            if check_old:
                ...
                conn.execute(text(f"EXEC sp_rename '{table}.review_uuid', 'review_id', 'COLUMN'"))
                conn.execute(text(f"ALTER TABLE {table} ALTER COLUMN review_id UNIQUEIDENTIFIER NOT NULL"))
            else:
        ...
        sys.stdout.flush()
        conn.execute(text("ALTER TABLE reviews ADD CONSTRAINT PK_reviews PRIMARY KEY (review_id)"))
        ...
        for table in detail_tables:
            if table == 'review_media':
                ...
                sys.stdout.flush()
                conn.execute(text(f"ALTER TABLE {table} ADD CONSTRAINT FK_{table}_reviews FOREIGN KEY (review_id) REFERENCES reviews(review_id) ON DELETE CASCADE"))
            else:
                print(f"Adding PK and FK to {table}...")
                sys.stdout.flush()
                conn.execute(text(f"ALTER TABLE {table} ADD CONSTRAINT PK_{table} PRIMARY KEY (review_id)"))
                conn.execute(text(f"ALTER TABLE {table} ADD CONSTRAINT FK_{table}_reviews FOREIGN KEY (review_id) REFERENCES reviews(review_id) ON DELETE CASCADE"))
