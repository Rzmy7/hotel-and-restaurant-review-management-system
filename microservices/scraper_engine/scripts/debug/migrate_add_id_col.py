...
def add_column():
    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            conn.execute(text("ALTER TABLE reviews ADD platform_review_id NVARCHAR(255) NULL"))
            conn.commit()
            print("Successfully added platform_review_id to reviews table.")
        except Exception as e:
            print(f"Note: Could not add column (it might already exist or table missing): {e}")

        print("Creating index IX_reviews_platform_id...")
        try:
            conn.execute(text("CREATE INDEX IX_reviews_platform_id ON reviews (source_id, platform_review_id)"))
            conn.commit()
