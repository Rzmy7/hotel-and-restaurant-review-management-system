...
def find_duplicate_review_ids(session: Session, detail_model: Type, source_id: str, grouping_columns: List) -> List[str]:
    """
    ...
    subq = (
        session.query(
            *grouping_columns,
            func.min(Review.created_at).label("min_created")
        )
    ...
    min_ids_query = (
        ...
        .join(Review, Review.review_id == detail_model.review_id)
        .join(subq, and_(
            *[getattr(detail_model, col.name) == getattr(subq.c, col.name) for col in grouping_columns],
