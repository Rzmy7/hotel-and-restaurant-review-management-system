...
def clean_agoda_duplicates(session: Session, source_id: str) -> int:
    ...
    duplicate_ids = find_duplicate_review_ids(session, AgodaReviewDetail, source_id, columns)
    removed = remove_duplicate_reviews(session, duplicate_ids)
    if removed > 0:
        logger.info(f"Removed {removed} duplicate Agoda reviews for source {source_id}.")
    return removed
