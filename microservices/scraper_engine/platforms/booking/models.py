...
def save_reviews_to_db(reviews, source_id: str) -> int:
    ...
    try:
        ...
        logger.info(f"Attempting to write {len(reviews)} Booking reviews for source_id={source_id}")
        ...
        for r in reviews:
            ...
            for attempt in range(1, 4):  # 3 attempts
                try:
                    ...
                    if attempt < 3:
                        logger.warning(f"Retry {attempt}/3 for Booking review {getattr(r, 'id', 'unknown')} due to: {e}")
                        time.sleep(1)  # Brief wait before retry
                    else:
                        logger.error(f"Fatal failure for Booking review {getattr(r, 'id', 'unknown')} after 3 attempts: {e}")
            
        session.commit()
        logger.info(f"Verified and committed {success_count}/{len(reviews)} Booking reviews for source_id={source_id}.")
