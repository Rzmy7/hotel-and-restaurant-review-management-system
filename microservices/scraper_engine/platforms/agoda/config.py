...
class AgodaSelectors(BaseModel):
    ...
    review_nationality_selector: str = (
        '[data-info-type="reviewer-name"] > span:last-child'
    )
    review_date_xpath: str = "//span[contains(text(), 'Reviewed')]"
    review_images_selector: str = (
        '[data-element-name="review-comment-ugc-thumbnail"] img, [data-picture-id] img'
    )
    ...
    open_reviews_selector_alt: str = (
        "button[data-element-name='read-more-reviews-on-recent-review-scores']"
    )
