...
def dismiss_popups(page):
    try:
        logger.info(
            "Sending Escape sequence and ambient click to softly clear passive overlays."
        )
...
def scrape_agoda(
    ...
    logger.info(
        f"Starting API scraper for URL: {url} (Headless: {headless}, Pages: {pages}, source_id: {source_id})"
    )
    ...
    try:
        ...
        while nav_attempt < max_nav_retries:
            try:
                nav_attempt += 1
                logger.info(
                    f"Navigating to Agoda URL (Attempt {nav_attempt}/{max_nav_retries}): {url}"
                )
                ...
                if nav_attempt >= max_nav_retries:
                    logger.error(
                        f"Failed to navigate after {max_nav_retries} attempts."
                    )
                    raise e

                wait_time = 5 * nav_attempt
                logger.warning(
                    f"Navigation attempt {nav_attempt} timed out or failed: {str(e)}. Retrying in {wait_time}s..."
                )
        ...
        try:
            ...
            if pages_val and not math.isnan(pages_val):
                ...
                logger.info(
                    f"Dynamically detected {total_pages_count} maximum pages from internal ul tag."
                )
        except Exception as e:
        ...
        if source_id:
            try:
                ...
                db_sess = get_session()
                rows = (
                    db_sess.query(Review.platform_review_id)
                    .filter(Review.source_id == str(source_id))
                    .all()
                )
                ...
                db_sess.close()
                logger.info(
                    f"Pre-loaded {len(seen_ids)} existing review IDs into memory cache for deduplication."
                )
            except Exception as e:
        ...
        consecutive_skips = (
            0  # Track pages with all-duplicate reviews to detect stuck pagination
        )
        ...
        while current_page <= end_page:
            ...
            if current_page < start_page:
                ...
                next_btn = page.locator(
                    '#reviewSection [data-element-name="review-paginator-next"], #reviewSection [aria-label="Next reviews page"]'
                ).first
                if next_btn.count() > 0:
                    next_btn.evaluate(
                        "element => { element.scrollIntoView({block: 'center'}); setTimeout(() => element.click(), 500); }"
                    )
                    time.sleep(3)
                else:
            ...
            if not new_reviews:
                ...
                if consecutive_skips >= 2:
                    logger.info(
                        f"Pagination stuck: same reviews appearing for {consecutive_skips + 1} consecutive pages. Reached true end of reviews."
                    )
                    break

                logger.info(
                    f"All {len(page_reviews)} reviews on page {current_page} are already in the database. Skipping to next page..."
                )
                ...
                logger.info(
                    f"Extracted {len(new_reviews)} unique reviews from page tab {current_page}."
                )
                ...
                if len(all_reviews) >= 20:
                    ...
                    logger.info(
                        f"Verified {verified_count}/{len(to_save)} Agoda reviews successfully persisted."
                    )
            ...
            if current_page < end_page:
                ...
                if next_btn.count() > 0:
                    logger.info(
                        f"Force clicking Page {target_page} button for extraction iteration."
                    )
                    next_btn.evaluate(
                        "element => { element.scrollIntoView({block: 'center'}); setTimeout(() => element.click(), 500); }"
                    )
                    ...
                    logger.info(
                        "Waiting for Javascript API to fetch and render reviews inline natively..."
                    )
                    time.sleep(5)
                    ...
                    try:
                        ...
                        if actual_page and actual_page == current_page:
                            logger.info(
                                f"Page verification: pagination did NOT advance (still on page {actual_page}). Reached true end of reviews."
                            )
                            break
                        elif actual_page:
                            logger.info(
                                f"Page verification: confirmed on page {actual_page}."
                            )
                    ...
                    logger.info(
                        f"Page tab {target_page} not found natively. We might have exhausted review pages."
                    )
                    break
            else:
                break
