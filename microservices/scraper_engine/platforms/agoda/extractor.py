...
class AgodaExtractor:
    ...
    def extract_reviews(self) -> List[Review]:
        logger.info(
            "Extracting reviews using Hybrid DOM + Regex from native #reviewSection inline DOM."
        )
        reviews = []
        ...
        for i in range(count):
            ...
            author_match = re.search(
                r'data-info-type="reviewer-name"[^>]*>.*?<strong>([^<]+)</strong>', html
            )
            if not author_match:
                continue
            ...
            # 2. Nationality
            nat_match = re.search(
                r'data-info-type="reviewer-name"[^>]*>.*?<span>([^<]+)</span></div>',
                html,
            )
            ...
            if not rating_match:
                rating_match = re.search(
                    r'data-selenium="review-score"[^>]*>([\d.]+)<', html
                )
            ...
            if not text_match:
                text_match = re.search(
                    r'data-selenium="review-body"[^>]*>([^<]+)<', html
                )
            if not text_match:
                text_match = re.search(
                    r'class="[^"]*Review-comment-bodyText[^"]*"[^>]*>([^<]+)<', html
                )
            ...
            try:
                ...
                img_elements = container.locator(
                    agoda_selectors.review_images_selector
                ).all()
                ...
                logger.debug(
                    f"Locator-based image extraction failed, falling back to regex: {e}"
                )

            if not images:
                # Fallback: regex search for data-picture-id and src in the same block
                img_matches = re.finditer(
                    r'data-picture-id="[^"]*".*?src="([^"]+)"', html, re.DOTALL
                )
                for m in img_matches:
                    full_src = (
                        m.group(1)
                        if not m.group(1).startswith("//")
                        else "https:" + m.group(1)
                    )
                    if full_src not in images:
                        images.append(full_src)
            ...
            reply_match = re.search(
                r'class="[^"]*Review-response-text[^"]*"[^>]*>([^<]+)<', html
            )
