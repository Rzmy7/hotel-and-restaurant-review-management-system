...
class TripAdvisorSelectors:
    ...
    REVIEW_CARD = (
        'div[data-test-target="HR_CC_CARD"], div[data-automation="reviewCard"]'
    )
    ...
    # Management Response
    REPLY_CONTAINER = (
        'div.swNpl, div[data-test-target="management-response"], div.WvSsn'
    )
    REPLY_TEXT = "div.fIrGe._T.bgMZj, div.MyMKp, span.yCeTE"
    REPLY = (
        'div.swNpl, div:has-text("Response from"), div:has-text("Management response")'
    )
    ...
    READ_MORE_BTN = (
        'span:has-text("Read more"), button.UikNM, button[aria-expanded="false"]'
    )
