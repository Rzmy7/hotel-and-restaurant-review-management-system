...
class SourceService:
    ...
    @staticmethod
    def finalize_and_replicate(
        url: str,
        primary_source_id: str,
        reviews: list,
        save_db_func,
        deduplicator_func=None,
        leftover_reviews: list = None,
    ):
        """
            ...
            for source in all_sources:
                ...
                try:
                    ...
                    if sid == primary_source_id:
                        ...
                        to_save = (
                            leftover_reviews
                            if leftover_reviews is not None
                            else reviews
                        )
                        if to_save:
                            logger.info(
                                f"Saving {len(to_save)} leftover reviews for primary source {sid}"
                            )
                            save_db_func(to_save, sid)
                        else:
                            logger.info(
                                f"No leftovers to save for primary source {sid}"
                            )
                    else:
                        # For companion sources, save the full set as they weren't part of the batch scrape
                        logger.info(
                            f"Replicating {len(reviews)} reviews to companion source {sid}"
                        )
                    ...
                    try:
                        if deduplicator_func:
                            ...
                            logger.info(
                                f"Deduplication complete. Removed {removed} duplicates."
                            )
                    ...
                    # Notify Completion
                    notify_backend_sync_status(
                        sid, "COMPLETED", new_review_count=new_count
                    )

                except Exception as inner_e:
                    logger.error(
                        f"Shared finalization failed for source {sid}: {inner_e}"
                    )
                    notify_backend_sync_status(
                        sid, "FAILED", error_message=str(inner_e)
                    )
    ...
    def broadcast_completed_no_data(url: str):
        ...
        try:
            ...
            for source in sources:
                notify_backend_sync_status(
                    source.source_id, "COMPLETED", new_review_count=0
                )
        except Exception as e:
