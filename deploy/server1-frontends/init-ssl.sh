#!/bin/bash
# ═══════════════════════════════════════════════════════════════════
# init-ssl.sh — Bootstrap SSL certificates for Server 1 (Frontends)
...
DOMAINS="reviewmate.live admin.reviewmate.live"
EMAIL="admin@reviewmate.live"   # Change to your real email
COMPOSE_FILE="docker-compose.yml"

echo "Starting SSL certificate setup..."

# Step 1: Start nginx with HTTP-only (it will fail on SSL, so use a temp config)
