...
class TestGetOrgsWithEnabledRules:
    def test_returns_distinct_org_ids(self):
        conn = MagicMock()
        conn.cursor().fetchall.return_value = [("org-1",), ("org-2",)]
        with patch.object(ars.pyodbc, "connect", return_value=conn):
            orgs = ars.get_orgs_with_enabled_rules()
    ...
    def test_returns_empty_when_no_rules(self):
        ...
        with patch.object(ars.pyodbc, "connect", return_value=conn):
            assert ars.get_orgs_with_enabled_rules() == []


class TestEvaluateAlertRulesJob:
    def test_job_evaluates_all_orgs_with_cooldown(self):
        with patch(
            "app.modules.scheduler.tasks.alert_evaluation_tasks.get_orgs_with_enabled_rules",
            return_value=["org-1", "org-2"],
        ), patch(
            "app.modules.scheduler.tasks.alert_evaluation_tasks.evaluate_and_notify",
            return_value=[{"severity": "warning", "title": "x"}],
        ) as ev:
    ...
    def test_job_noop_when_no_enabled_rules(self):
        with patch(
            "app.modules.scheduler.tasks.alert_evaluation_tasks.get_orgs_with_enabled_rules",
            return_value=[],
        ), patch(
    ...
    def test_job_org_failure_does_not_stop_run(self):
        with patch(
            "app.modules.scheduler.tasks.alert_evaluation_tasks.get_orgs_with_enabled_rules",
            return_value=["org-1", "org-2"],
        ), patch(
    ...
    def test_job_fetch_failure_logs_and_returns(self):
        with patch(
            "app.modules.scheduler.tasks.alert_evaluation_tasks.get_orgs_with_enabled_rules",
