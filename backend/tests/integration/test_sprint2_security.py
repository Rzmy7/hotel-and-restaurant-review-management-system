...
class TestSourceSetupSecurity:
    """Security tests for the Source setup endpoints."""

    def test_get_setup_sources_success(self, security_client):
        with patch("sqlalchemy.orm.Session.execute"):
            response = security_client.get(f"/api/setup/sources?organization_id={TEST_OWNED_ORG_ID}")
            assert response.status_code == status.HTTP_200_OK

    def test_get_setup_sources_unauthorized(self, security_client):
        response = security_client.get(f"/api/setup/sources?organization_id={TEST_UNOWNED_ORG_ID}")
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_connect_setup_source_success(self, security_client):
        payload = {
            "source_name": "Agoda",
            "organization_id": TEST_OWNED_ORG_ID,
            "source_url": "https://agoda.com/hotel/test",
        }
        with patch("sqlalchemy.orm.Session.execute"):
            response = security_client.post("/api/setup/sources/connect", json=payload)
            assert response.status_code == status.HTTP_200_OK

    def test_connect_setup_source_unauthorized(self, security_client):
        payload = {
            "source_name": "Agoda",
            "organization_id": TEST_UNOWNED_ORG_ID,
            "source_url": "https://agoda.com/hotel/test",
    ...
    def test_disconnect_setup_source_success(self, security_client):
        payload = {
            "source_name": "Agoda",
            "organization_id": TEST_OWNED_ORG_ID,
            "source_url": "https://agoda.com/hotel/test",
        ...
        mock_db_res.fetchone.return_value = ("source-123", True)
        with patch("sqlalchemy.orm.Session.execute", return_value=mock_db_res):
            response = security_client.post("/api/setup/sources/disconnect", json=payload)
            assert response.status_code == status.HTTP_200_OK

    def test_disconnect_setup_source_unauthorized(self, security_client):
        payload = {
            "source_name": "Agoda",
            "organization_id": TEST_UNOWNED_ORG_ID,
            "source_url": "https://agoda.com/hotel/test",
