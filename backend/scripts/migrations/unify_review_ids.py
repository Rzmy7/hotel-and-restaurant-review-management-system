...
def migrate():
    ...
    with pyodbc.connect(connection_string) as conn:
        ...
        print(
            "Updating review_media.review_id to match scraper_review_id (for UUIDs only)..."
        )
        cursor.execute("""
            UPDATE rm 
        ...
        print(
            "Updating processed_review.id to match scraper_review_id (for UUIDs only)..."
        )
        cursor.execute("""
        ...
        cursor.execute(
            "ALTER TABLE processed_review ADD CONSTRAINT PK_processed_review PRIMARY KEY (id)"
        )

        # 3. Re-add Foreign Key to review_media
        print("Adding FK to review_media...")
        cursor.execute(
            "ALTER TABLE review_media ADD CONSTRAINT FK_review_media_processed_review FOREIGN KEY (review_id) REFERENCES processed_review(id) ON DELETE CASCADE"
        )
