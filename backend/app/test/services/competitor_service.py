...
genai_client = genai.Client(api_key=GENAI_KEY, http_options={"api_version": "v1"})


# ------------------------------------------------------------------
# 2. Database Operations — Competitors CRUD
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 3. Database Operations — Competitor Reviews
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 4. Competitor Scraping + AI Processing
# ------------------------------------------------------------------
...
def process_competitor_scrape(competitor_id: int, url: str, headless: bool = True) -> None:
    ...
    """
    from scraping.booking import scrape_booking_for_competitor
    ...
    try:
        # Step 1: Scrape raw reviews (returns list of Review dataclass objects)
...
def get_category_scores(table: str, where_clause: str = "", params: list = None) -> Dict[str, float]:
    ...
    Categories are stored as JSON arrays. We parse them and average ratings.
    """
    if params is None:
        params = []
...
def get_comparison_data(competitor_id: int) -> Dict:
    """
    Build the full comparison dataset between the user's hotel and a competitor.
    Used by the CompetitorComparison page.
    """
    ...
    # Category scores (aspect comparison for radar chart)
    ...
    # Merge all categories into a unified aspect list
    ...
    # Merge trends into unified timeline
    ...
    trend_data = [
        ...
        for m in trend_months[-7:]  # Last 7 months
    ]
...
def get_rankings_data() -> Dict:
    ...
    # Add user's hotel
    entries.append({
        "name": "My Hotel",  # Frontend can override with org name
        "isYou": True,
    ...
    # Find your rank and top performer
