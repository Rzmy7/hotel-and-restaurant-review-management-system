...
def platform_visuals(platform_name: str) -> tuple[str, str]:
    ...
    return mapping.get(
        key, (platform_name[:2].upper() if platform_name else "P", "#6B7280")
    )
...
def normalize_sql_type(raw_type: str) -> str:
    ...
    if decimal_match:
        ...
        if precision < 1 or precision > 38:
            raise HTTPException(
                status_code=400, detail="DECIMAL precision must be between 1 and 38"
            )
        if scale < 0 or scale > precision:
            raise HTTPException(
                status_code=400, detail="DECIMAL scale must be between 0 and precision"
            )
        return f"DECIMAL({precision},{scale})"

    length_match = re.fullmatch(
        r"(varbinary|varchar|nvarchar|char|nchar)\((max|\d{1,5})\)", value
    )
    if length_match:
        ...
        if length_value <= 0:
            raise HTTPException(
                status_code=400, detail=f"{base_type} length must be greater than 0"
            )
        if base_type in {"NCHAR", "NVARCHAR"} and length_value > 4000:
            raise HTTPException(
                status_code=400, detail=f"{base_type} length cannot exceed 4000"
            )
        if base_type in {"CHAR", "VARCHAR", "VARBINARY"} and length_value > 8000:
            raise HTTPException(
                status_code=400, detail=f"{base_type} length cannot exceed 8000"
            )
...
def normalize_table_attributes(
    attributes: list[ScrapingTableAttributePayload],
) -> list[dict[str, str | bool]]:
    if not attributes:
        raise HTTPException(
            status_code=400, detail="At least one table attribute is required"
        )
    ...
    for attr in attributes:
        ...
        if normalized_name in seen_names:
            raise HTTPException(
                status_code=400, detail=f"Duplicate attribute name '{column_name}'"
            )
...
def fetch_table_attributes(
    cursor: pyodbc.Cursor, table_name: str | None
) -> list[dict[str, str | bool]]:
    sanitized = (table_name or "").strip()
    if (
        not sanitized
        or not is_valid_sql_identifier(sanitized)
        or not table_exists(cursor, sanitized)
    ):
        return []
    ...
    return [
        {
            "name": str(row[0]),
            "type": sql_type_from_information_schema_row(
...
def create_dynamic_platform_table(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create table '{table_name}': {exc}"
        ) from exc

    if not table_exists(cursor, table_name):
        raise HTTPException(
            status_code=500, detail=f"Table '{table_name}' was not created"
        )
...
def drop_dynamic_platform_table(cursor: pyodbc.Cursor, table_name: str | None) -> None:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to drop table '{sanitized}': {exc}"
        ) from exc
...
def sync_dynamic_platform_table(
    ...
    if existing_table_name and not is_valid_sql_identifier(existing_table_name):
        raise HTTPException(
            status_code=500, detail="Stored platform table name is invalid"
        )
    ...
    if not existing_table_name:
        if table_exists(cursor, target_table_name):
            raise HTTPException(
                status_code=409, detail=f"Table '{target_table_name}' already exists"
            )
    ...
    if existing_table_name.lower() != target_table_name.lower():
        ...
        try:
            cursor.execute(
                "EXEC sp_rename ?, ?", (f"dbo.{existing_table_name}", target_table_name)
            )
        except Exception as exc:
            raise HTTPException(
                status_code=500,
    ...
    if not table_exists(cursor, target_table_name):
        raise HTTPException(
            status_code=500,
            detail=f"Table '{target_table_name}' was not found after rename",
        )
    ...
    row_count = int(
        execute_query(
            cursor, f"SELECT COUNT(1) FROM dbo.[{target_table_name}]"
        ).fetchone()[0]
        or 0
    )
    ...
    for key, existing in existing_by_lower.items():
        ...
        try:
            cursor.execute(
                f"ALTER TABLE dbo.[{target_table_name}] DROP COLUMN [{existing_name}]"
            )
        except Exception as exc:
            raise HTTPException(
                status_code=500,
...
def find_platform_row(
    cursor: pyodbc.Cursor, platform_id: str, select_cols_sql: str
) -> pyodbc.Row | None:
    try:
        pid = int(platform_id)
...
def fetch_platforms_from_db() -> list[dict[str, object]]:
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch platforms from database: {exc}"
        ) from exc
...
def get_platform_details_from_db(platform_id: str) -> dict[str, object]:
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch platform details: {exc}"
        ) from exc


def create_platform_in_db(
    ...
    fetching_type = (
        (payload.fetchingType or "SCRAPING").strip().upper()
        if hasattr(payload, "fetchingType") and payload.fetchingType
        else "SCRAPING"
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create platform: {exc}"
        ) from exc
...
def update_platform_in_db(
    ...
    base_url = payload.baseUrl.strip() if payload.baseUrl else None
    fetching_type = (
        (payload.fetchingType or "").strip().upper()
        if hasattr(payload, "fetchingType") and payload.fetchingType
        else None
    )
    t_name = (payload.tableName or "").strip()
    ...
    try:
        with pyodbc.connect(get_connection_string()) as connection:
            ...
            if (
                t_name
                and t_name.lower() != current_table.lower()
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update platform: {exc}"
        ) from exc
...
def scraping_backend_get(path: str) -> dict | list:
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Scraping backend request failed for {path}: {exc}"
        ) from exc
    try:
        return response.json()
    except ValueError as exc:
        raise HTTPException(
            status_code=502, detail=f"Invalid JSON from scraping backend for {path}"
        ) from exc
...
def scraping_backend_post(path: str, json_body: dict | None = None) -> dict | list:
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Scraping backend POST failed for {path}: {exc}"
        ) from exc
    try:
        return response.json()
    except ValueError as exc:
...
def format_duration_from_created_at(
    created_at_str: str | None, ended_at_str: str | None = None
) -> str:
