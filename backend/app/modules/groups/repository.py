...
All heavy SQL (analytics) uses SQLAlchemy text() so it runs directly on MSSQL.
"""
...
def list_user_groups(db: Session, user_id: str) -> List[dict]:
    """Return all groups where the user is a member (owner or member)."""
    rows = db.execute(
        text("""
            SELECT
                g.group_id,
...
def get_group_detail(db: Session, group_id: str, user_id: str) -> Optional[dict]:
    row = db.execute(
        text("""
            SELECT
                g.group_id,
                ...
                (SELECT COUNT(*) FROM group_member gm2 WHERE gm2.group_id = g.group_id) AS member_count
            FROM [group] g
            JOIN group_member gm ON gm.group_id = g.group_id AND gm.user_id = :uid
...
def add_member(db: Session, group_id: str, user_id: str, role: str = "GROUP_MEMBER") -> GroupMember:
...
def get_member(db: Session, group_id: str, user_id: str) -> Optional[GroupMember]:
    return (
        db.query(GroupMember)
        .filter(GroupMember.group_id == group_id, GroupMember.user_id == user_id)
        .first()
...
def get_user_group_role(db: Session, group_id, user_id) -> Optional[str]:
    member = get_member(db, str(group_id), str(user_id))
    ...
    rows = db.execute(
        text("""
            SELECT
                ...
                iu.first_name AS invitee_first,
                iu.last_name  AS invitee_last,
                iu.email      AS invitee_email
            FROM group_invite gi
            JOIN [user] ib ON ib.user_id = gi.invited_by
            LEFT JOIN [user] iu ON iu.user_id = gi.invited_user_id
...
def list_my_pending_invites(db: Session, user_id: str) -> List[dict]:
    """Return all pending invites sent TO the current user."""
    rows = db.execute(
        text("""
            SELECT
                gi.invite_id,
                ...
                ib.first_name AS inviter_first,
                ib.last_name  AS inviter_last,
                NULL AS invitee_first,
                NULL AS invitee_last,
                NULL AS invitee_email
            FROM group_invite gi
            JOIN [group] g ON g.group_id = gi.group_id
            JOIN [user] ib ON ib.user_id = gi.invited_by
            WHERE gi.invited_user_id = :uid
...
def has_pending_invite(db: Session, group_id: str, user_id: str) -> bool:
    row = db.execute(
        text("""
            SELECT COUNT(*) FROM group_invite
            WHERE group_id = :gid AND invited_user_id = :uid AND status = 'pending'
        """),
        {"gid": group_id, "uid": user_id},
    ).scalar()
    return row > 0
...
def _invite_row_to_dict(r) -> dict:
    return {
        ...
        "invited_user_id": str(r.invited_user_id) if r.invited_user_id else None,
        "invited_user_name": f"{r.invitee_first or ''} {r.invitee_last or ''}".strip() or None,
        "invited_user_email": r.invitee_email,
...
def search_organizations(db: Session, query: str, limit: int = 20) -> List[dict]:
    """Search organizations for the invite modal."""
    rows = db.execute(
        text("""
            SELECT TOP (:lim)
                o.organization_id,
...
def get_org_owner_user_id(db: Session, organization_id: str) -> Optional[str]:
    """Return the user_id of the organization owner (tenant)."""
    row = db.execute(
        text("""
            SELECT t.tenant_id
            FROM organization o
...
def get_group_analytics(db: Session, group_id: str) -> dict:
    """Aggregate review and member statistics for a group."""
    ...
    review_agg = db.execute(
        text("""
            ...
            JOIN group_member gm ON gm.user_id = o.tenant_id
            WHERE gm.group_id = :gid
    ...
    invite_stats_row = db.execute(
        text("""
            ...
            FROM group_invite
            WHERE group_id = :gid AND invite_type = 'user'
        """),
        {"gid": group_id},
    ).fetchone()

    # Recent members (last 5 joins)
    recent_rows = db.execute(
        text("""
            SELECT TOP 5
                u.user_id, u.first_name, u.last_name, u.email, u.profile_image_url,
    ...
    # Member organizations with review counts
    org_rows = db.execute(
        text("""
            SELECT
                o.organization_id,
            ...
            LEFT JOIN processed_review pr
               ...
               AND ISNULL(pr.status, '') != 'error'
            WHERE gm.group_id = :gid
            GROUP BY o.organization_id, o.organization_name, u.first_name, u.last_name
