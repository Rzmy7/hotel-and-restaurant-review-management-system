...
def _get_response_metrics(db: Session, org_id: str, curr_start, prev_start, prev_end):
    """
    ...
    sql_curr = """
        ...
        WHERE s.organization_id = :org_id AND r.reviewDate >= CAST(:curr_start AS DATE)
    """
    row = db.execute(text(sql_curr), {"org_id": org_id, "curr_start": curr_start}).fetchone()
    curr_total = row[0] if row and row[0] is not None else 0
    curr_replied = row[1] if row and row[1] is not None else 0
    ...
    sql_prev = """
        ...
        FROM dbo.processed_review r
        JOIN dbo.source s ON r.source_id = s.source_id
        WHERE s.organization_id = :org_id AND r.reviewDate >= CAST(:prev_start AS DATE) AND r.reviewDate < CAST(:prev_end AS DATE)
    """
    row = db.execute(text(sql_prev), {"org_id": org_id, "prev_start": prev_start, "prev_end": prev_end}).fetchone()
    prev_total = row[0] if row and row[0] is not None else 0
    prev_replied = row[1] if row and row[1] is not None else 0
    ...
    try:
        ...
        """
        row = db.execute(text(sql_avg), {"org_id": org_id, "curr_start": curr_start}).fetchone()
        avg_hours = row[0] if row and row[0] is not None else None
...
def _compute_rating_impact(db: Session, org_id: str, curr_start) -> str:
    """
    ...
    try:
        ...
        """
        row = db.execute(text(sql), {"org_id": org_id, "curr_start": curr_start}).fetchone()
        replied = float(row[0] or 0) if row else 0.0
        unreplied = float(row[1] or 0) if row else 0.0
...
def get_insights(
    ...
    try:
        ...
        # ── KPI Metrics ──────────────────────────────────────────
        try:
            curr_avg    = get_avg_rating(db, org_id, start_date=curr_start)
        except Exception as e:
            raise RuntimeError(f"[get_avg_rating#1] {e}") from e
        try:
            prev_avg    = get_avg_rating(db, org_id, start_date=prev_start, end_date=prev_end) if curr_start else 0
        except Exception as e:
            raise RuntimeError(f"[get_avg_rating#2] {e}") from e
        try:
            curr_total  = get_review_count(db, org_id, start_date=curr_start)
        except Exception as e:
            raise RuntimeError(f"[get_review_count#1] {e}") from e
        try:
            prev_total  = get_review_count(db, org_id, start_date=prev_start, end_date=prev_end) if curr_start else 0
        except Exception as e:
            raise RuntimeError(f"[get_review_count#2] {e}") from e
        try:
            response_rate = get_response_rate(db, org_id, start_date=curr_start)
        except Exception as e:
        ...
        try:
            sentiment_series = get_weekly_sentiment_series(db, org_id, period_days=period)
        except Exception as e:
            import traceback
            traceback.print_exc()
        ...
        # ── Rating Distribution ───────────────────────────────────
        try:
            raw_dist = get_rating_distribution(db, org_id, period_days=period)
        except Exception as e:
        ...
        try:
            cat_raw = get_category_performance(db, org_id, period_days=period)
        except Exception as e:
        ...
        # ── Source Breakdown ─────────────────────────────────────
        try:
            src_raw = get_source_comparison_metrics(db, org_id, period_days=period)
        except Exception as e:
        ...
        # ── Keywords ─────────────────────────────────────────────
        try:
            keywords = get_keywords(db, org_id, period_days=period)
        except Exception as e:
        ...
        try:
            heatmap_result = get_review_volume_heatmap(db, org_id, period_days=period)
            if heatmap_result:
        ...
        return {
            ...
            "responseMetrics": {
                ...
                "ratingImpact": _compute_rating_impact(db, org_id, curr_start),
            },
