...
def get_review_metrics(cursor: pyodbc.Cursor) -> Dict[str, Any]:
    ...
    # Processed reviews (reviews that have been through AI analysis)
    cursor.execute("SELECT COUNT(*) FROM dbo.processed_review WHERE [status] = 'processed'")
        ...
        if pending > 50:
            alerts.append({
                ...
                "message": (
                    ...
                    "Ensure an active LLM Model is configured and quota is available."
                ),
