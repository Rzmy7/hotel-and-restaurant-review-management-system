...
@retry(
    ...
    before_sleep=lambda retry_state: logger.warning(
        f"Gemini API busy (503/Error). Retrying in {retry_state.next_action.sleep} seconds... (Attempt {retry_state.attempt_number})"
    ),
)
    ...
    try:
        ...
        if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
            try:
                ...
                logger.warning(
                    "Could not import notification helper to notify admin of Gemini quota issue."
                )
            except Exception as notify_err:
                logger.warning(
                    f"Failed to trigger admin notification for Gemini quota: {notify_err}"
                )
