...
def get_stuck_tasks(
    db: Session = Depends(get_db), internal: bool = Depends(verify_internal_api_key)
):
...
def get_organizations(
    tenant_id: uuid.UUID, db: Session = Depends(get_db), user=Depends(get_current_user)
):
...
def export_sync_logs(
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=sync_history_{organization_id}.csv"
        },
    )


@router.delete(
    "/organizations/{organization_id}/sync-logs/clear",
