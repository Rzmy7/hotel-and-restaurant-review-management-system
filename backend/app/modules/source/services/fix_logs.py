    ...
    try:
        ...
        for log in logs_to_fix:
            ...
            if source:
                platform = (
                    db.query(Platform)
                    .filter(Platform.platform_id == source.platform_id)
                    .first()
                )
                if platform:
            ...
            if not log.activity_details:
                if log.activity_type == "SYNC_COMPLETED":
                    ...
                    log.activity_details = (
                        f"Synchronization process initiated for {platform_name}."
                    )
                elif log.activity_type == "SYNC_QUEUED":
                    log.activity_details = (
                        f"Source {platform_name} placed in high-priority sync queue."
                    )
                    ...
                    log.activity_details = (
                        f"AI analysis completed for {log.reviews_fetched or 0} reviews."
                    )
                else:
