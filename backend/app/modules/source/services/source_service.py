...
def get_platforms(db: Session, include_inactive: bool = False) -> List[PlatformRead]:
    ...
    return [
        PlatformRead(
            ...
            success_rate=(
                round((p.success_sync_count / p.num_of_syncs) * 100, 2)
                if p.num_of_syncs > 0
                else 0.0
            ),
...
) -> List[OrganizationSource]:
    return (
        db.query(OrganizationSource)
        .filter(OrganizationSource.tenant_id == tenant_id)
        .all()
    )
...
def get_organization_sources_with_stats(
    ...
    org = (
        ...
        .filter(OrganizationSource.organization_id == organization_id)
        .first()
    )
...
def create_source(db: Session, source_data: SourceCreate) -> SourceRead:
    ...
    platform = (
        db.query(Platform)
        .filter(Platform.platform_id == source_data.platform_id)
        .first()
    )
    if not platform:
    ...
    source = (
        ...
        .filter(SourceSource.source_id == new_source.source_id)
        .first()
    )
...
def update_source(
    db: Session, source_id: uuid.UUID, source_data: SourceUpdate
) -> SourceRead:  #
    source = (
        db.query(SourceSource)
        .options(joinedload(SourceSource.platform))
    ...
    if (
        "platform_id" in update_data
        and update_data["platform_id"] != source.platform_id
    ):
        new_platform = (
            db.query(Platform)
            .filter(Platform.platform_id == update_data["platform_id"])
            .first()
        )
        if not new_platform:
    ...
    if (
        "fetching_frequency" in update_data
        and update_data["fetching_frequency"] != source.fetching_frequency
    ):
        base_time = source.last_synced_at or source.created_at
        source.next_synced_at = calculate_next_sync_time(
            base_time, update_data["fetching_frequency"]
        )
...
def get_tenant_sources(db: Session, tenant_id: uuid.UUID) -> List[SourceRead]:
    sources = (
        ...
        .join(
            OrganizationSource,
            SourceSource.organization_id == OrganizationSource.organization_id,
        )
        .options(joinedload(SourceSource.platform))
        .filter(OrganizationSource.tenant_id == tenant_id)
        .all()
...
def get_sync_logs(
    ...
    query = (
        db.query(SyncLogSource)
        .join(SourceSource, SyncLogSource.source_id == SourceSource.source_id)
        .options(joinedload(SyncLogSource.source).joinedload(SourceSource.platform))
        .filter(SourceSource.organization_id == organization_id)
    )
    ...
    logs = (
        query.order_by(SyncLogSource.timestamp.desc()).offset(skip).limit(limit).all()
    )
    ...
    if request.status == SyncStatus.COMPLETED:
        ...
        if background_tasks:
            background_tasks.add_task(
                start_ingestion_and_processing_flow, source_id, sync_log.log_id
            )
        ...
        try:
            ...
            org = (
                ...
                .filter(Organization.organization_id == source.organization_id)
                .first()
            )
...
def trigger_sync(db: Session, source_id: uuid.UUID):
    ...
    platform_key = (
        source.platform.platform_name.lower()
        .replace(" reviews", "")
        .replace(".com", "")
    )
    ...
    try:
        ...
        with httpx.Client() as client:
            ...
            if resp.status_code in [200, 201, 202]:
                ...
                if job_id:
                    from app.modules.source.services.sync_socket_manager import (
                        sync_socket_manager,
                    )
...
def prune_activities(db: Session, organization_id: uuid.UUID):
    ...
    source_ids_query = (
        db.query(SourceSource.source_id)
        .filter(SourceSource.organization_id == organization_id)
        .all()
    )
    ...
    total_count = (
        db.query(SyncLogSource).filter(SyncLogSource.source_id.in_(source_ids)).count()
    )
    ...
    subquery = (
        db.query(SyncLogSource.log_id)
        .filter(SyncLogSource.source_id.in_(source_ids))
        .order_by(SyncLogSource.timestamp.desc())
        .limit(100)
        .all()
    )
    ...
    db.query(SyncLogSource).filter(
        SyncLogSource.source_id.in_(source_ids), ~SyncLogSource.log_id.in_(latest_ids)
            ...
            try:
                ...
                if org and org.tenant_id:
                    ...
                    message = (
                        activity_details
                        or error_message
                        or f"An important activity occurred for {source.platform.platform_name}."
                    )
                    notification_type = (
                        "error"
                        if status == "Failed" or activity_type == "SOURCE_REMOVED"
                        else "info"
                    )
                    send_notification(
                        str(org.tenant_id), title, message, notification_type
                    )
            except Exception:
                pass  # Best-effort
...
def delete_organization_logs(db: Session, organization_id: uuid.UUID):
    ...
    # Delete the rest
    db.query(SyncLogSource).filter(SyncLogSource.log_id.notin_(logs_to_keep)).join(
        SourceSource
    ).filter(SourceSource.organization_id == organization_id).delete(
