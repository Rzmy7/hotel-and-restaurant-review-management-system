...
def normalize_signup_email(email: str) -> str:
    ...
    if not is_realistic_domain(domain):
        raise HTTPException(
            status_code=400,
            detail="Please enter a valid email address with a recognized domain.",
        )
...
def validate_signup_name(name: str) -> str:
    ...
    if len(normalized) < 2:
        raise HTTPException(
            status_code=400, detail="Full name must be at least 2 characters."
        )
    if len(normalized) > 100:
        raise HTTPException(
            status_code=400, detail="Full name must be at most 100 characters."
        )
