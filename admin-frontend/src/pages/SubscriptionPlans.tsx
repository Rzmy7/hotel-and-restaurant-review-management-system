...
const PlanIcon: React.FC<{ name: PlanIconName; className?: string }> = ({
  name,
  className = "",
}) => {
...
const ensureAllFeatures = (
  ...
  const planFeatureMap = new Map(
    planFeatures.map((feature) => [feature.id, feature]),
  );
...
const PlanCard: React.FC<PlanCardProps> = ({
  ...
  return (
    <div
      className={`relative bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col transition-all duration-200 hover:shadow-md ${!plan.isActive ? "opacity-60" : ""}`}
    >
      ...
      <div className="flex-1 p-5">
        <ul className="space-y-2.5">
          {plan.features.map((feature) => (
            ...
            >
              <div
                className={`mt-0.5 flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${feature.enabled ? "bg-green-100" : "bg-gray-100 dark:bg-slate-700"}`}
              >
                {feature.enabled ? (
              ...
              <span
                className={`text-sm leading-snug ${feature.enabled ? "text-gray-700 dark:text-slate-200" : "text-gray-400 dark:text-slate-500 line-through"}`}
              >
                {feature.name}
                {feature.enabled &&
...
const PlanModal: React.FC<PlanModalProps> = ({
  ...
  const updateFeature = (
    featureId: string,
    patch: Partial<SubscriptionPlanFeature>,
  ) => {
    setForm((prev) => ({
      ...prev,
        ...
        >
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    value={form.iconName}
                    onChange={(e) =>
                      updateField("iconName", e.target.value as PlanIconName)
                    }
          ...
          <section>
            ...
            <p className="text-xs text-gray-400 dark:text-slate-500 mt-2">
              Set price to 0 to display "Free". Annual price is the per-month
              cost when billed yearly.
            </p>
          </section>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm font-medium ${feature.enabled ? "text-gray-700 dark:text-slate-200" : "text-gray-400 dark:text-slate-500"}`}
                      >
                        {feature.name}
                      </p>
                      {feature.description && (
...
export const SubscriptionPlans: React.FC = () => {
  ...
  const openEdit = (plan: SubscriptionPlan) => {
    setEditingPlan({
      ...plan,
      features: ensureAllFeatures(plan.features, featuresCatalog).map(
        (feature) => ({ ...feature }),
      ),
    });
    setModalMode("edit");
    ...
    try {
      ...
      if (modalMode === "add") {
        ...
        setPlans((prev) => [
          ...
          {
            ...created,
            features: ensureAllFeatures(created.features, featuresCatalog),
          },
        ]);
  ...
  const handleToggleActive = async (plan: SubscriptionPlan) => {
    ...
    try {
      ...
      setPlans((prev) =>
        prev.map((existing) => (existing.id === plan.id ? updated : existing)),
      );
    } catch (error) {
      const message =
        error instanceof Error
        ...
        {[
          {
            label: "Total Plans",
            value: plans.length,
            sub: `${plans.length - activePlans} inactive`,
          },
          {
            label: "Active Plans",
            value: activePlans,
          ...
          <div
            key={stat.label}
            className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm px-5 py-4"
          >
