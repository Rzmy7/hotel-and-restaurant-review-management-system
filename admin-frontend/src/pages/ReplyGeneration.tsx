...
export const ReplyGeneration: React.FC = () => {
  const [settings, setSettings] =
    useState<ReplyGenerationSettings>(defaultSettings);
  const [loading, setLoading] = useState(true);
  const [saveState, setSaveState] = useState<
    "idle" | "saving" | "saved" | "error"
  >("idle");
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [googleTestState, setGoogleTestState] = useState<
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      try {
        ...
        setErrorMessage(
          "Failed to load settings. Using defaults until you save.",
        );
  ...
  const updateField = <K extends keyof ReplyGenerationSettings>(
    key: K,
    value: ReplyGenerationSettings[K],
  ) => {
  ...
  const handleTestGoogleApiKey = async () => {
    ...
    try {
      const result = await settingsService.testReplyGenerationApiKey({
        provider: "google",
        apiKey,
        model,
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const saved =
        await settingsService.updateReplyGenerationSettings(settings);
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Failed to save reply generation settings.",
      );
    }
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 max-w-2xl">
                Organize model behavior, prompt context depth, and provider
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 md:p-6 shadow-sm space-y-5">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                  Select a Google Gemini model for reply generation.
                </p>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-4">
              ...
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3">
                  <ToggleSwitch
                    ...
                    onChange={(checked) =>
                      updateField("useEmbeddingRules", checked)
                    }
                    label="Use embedding search rules"
                  />
                  <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                    Include matched rule context in generated replies.
                  </p>
                </div>
                <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3">
                  <ToggleSwitch
                    ...
                    onChange={(checked) =>
                      updateField("useSimilarReviews", checked)
                    }
                    label="Use similar reviews"
                  />
                  <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                    Include embedding-matched reviews in the prompt.
                  </p>
                </div>
              </div>
              <div className="mt-4 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3">
                ...
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                  Controls how many embedding-matched reviews are injected when
                  similar reviews are enabled.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 md:p-6 shadow-sm space-y-4">
            ...
            <div className="rounded-xl border border-sky-200 dark:border-sky-900/50 bg-sky-50/50 dark:bg-sky-900/20 p-4 space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  {googleTestState === "testing"
                    ? "Testing Google..."
                    : "Test Google Key"}
                </button>
                {googleTestMessage && (
                  <span
                    className={`text-sm ${googleTestState === "success" ? "text-emerald-600" : "text-rose-600"}`}
                  >
                    {googleTestMessage}
                  </span>
                )}
        ...
        <div>
          {saveState === "saved" && (
            <p className="text-sm text-emerald-600 dark:text-emerald-400">
              Reply generation settings saved.
            </p>
          )}
          ...
          {saveState === "idle" && !errorMessage && (
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Save after changing model, context depth, or provider credentials.
            </p>
          )}
        </div>
        <button
