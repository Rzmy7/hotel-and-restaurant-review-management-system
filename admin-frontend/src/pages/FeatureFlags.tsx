...
export const FeatureFlags: React.FC = () => {
  ...
  const toggleStatus = async (id: string) => {
    ...
    const nextStatus =
      currentFlag.status === "Enabled" ? "Disabled" : "Enabled";
    setFlags((prevFlags) =>
      prevFlags.map((flag) =>
    ...
    try {
      const updated = await featureFlagsService.updateFeatureFlag(
        currentFlag.key,
        {
          status: nextStatus,
      ...
      setFlags((prevFlags) =>
        prevFlags.map((flag) => (flag.id === id ? updated : flag)),
      );
    } catch {
      setFlags((prevFlags) =>
        prevFlags.map((flag) =>
          ...
          >
            ...
            <div className="flex items-center gap-4 ml-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span
                  className={`text-sm font-medium min-w-[60px] transition-colors ${
                    flag.status === "Enabled"
                      ? "text-blue-600"
