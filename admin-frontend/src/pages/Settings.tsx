...
export const Settings: React.FC = () => {
  ...
  const handleSaveGeneralSettings = async () => {
    if (!settings || generalSaveState === "saving") return;
    ...
    try {
      ...
      setGeneralSaveError(error instanceof Error ? error.message : "Failed to save general settings.");
    }
  };
  ...
  const handleSaveSecuritySettings = async () => {
    ...
    try {
      ...
      setSecuritySaveError(error instanceof Error ? error.message : 'Failed to save security settings.');
    }
  };

  const handleSaveAdminProfile = async () => {
    if (adminProfileSaveState === "saving") return;
    ...
    try {
      ...
      setAdminProfileSaveState("error");
      setAdminProfileError(error instanceof Error ? error.message : "Failed to save admin profile.");
    }
  };
  ...
  const handleChangePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await settingsService.changeAdminPassword({ currentPassword, newPassword });
      ...
      setPasswordModalSaveState("error");
      setPasswordModalError(error instanceof Error ? error.message : "Failed to change password.");
    }
  };

  if (loading || !settings) return <LoadingSpinner />;

  const timezoneOptions = TIMEZONE_OPTIONS.some(o => o.value === settings.timezone) 
    ? TIMEZONE_OPTIONS 
    ...
    <div className="space-y-6 pt-4">
          ...
          <div className="grid grid-cols-2 gap-4">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`flex-1 flex flex-col items-center gap-1.5 px-3 py-2.5 rounded-xl border-2 transition-all duration-200 ${
                      theme === value ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300" : "border-gray-200 dark:border-slate-600 text-gray-600 dark:text-slate-400"
                    }`}
                  >
                    <Icon size={18} />
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-4 flex flex-col justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-slate-400">Enable to prevent users from accessing the platform</p>
              </div>
              <div className="flex justify-end">
                <ToggleSwitch checked={maintenanceMode} onChange={handleMaintenanceChange} disabled={isMaintenanceSaving} />
              </div>
            </div>
          </div>

          {maintenanceError && <Alert type="error" message={maintenanceError} />}
          {generalSaveError && <Alert type="error" message={generalSaveError} />}
      ...
      {activeTab === "security" && (
        <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-slate-400">Require 2FA for all admin accounts</p>
                </div>
                <ToggleSwitch
                  checked={securitySettings.requireTwoFactorAuth}
                  onChange={(checked) => setSecuritySettings(p => ({ ...p, requireTwoFactorAuth: checked }))}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSecuritySettings(p => ({ ...p, userSessionTimeoutMinutes: Number(e.target.value) }))}
                    className="w-full px-3 py-2 border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSecuritySettings(p => ({ ...p, adminSessionTimeoutMinutes: Number(e.target.value) }))}
                    className="w-full px-3 py-2 border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    <option value={15}>15 Minutes</option>
                    <option value={30}>30 Minutes</option>
          ...
          {securitySaveState === 'saved' && <Alert type="success" message="Security settings saved." />}
          {securitySaveError && <Alert type="error" message={securitySaveError} />}
      ...
      {activeTab === "notifications" && (
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-4">
            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-4">Admin Notifications</h2>
            <div className="space-y-4">
              {[
                { label: "API Limit Reaching", key: "notifyApiLimitReaching" },
                { label: "Server Overloading", key: "notifyServerOverloading" },
                { label: "Server Connection Failed", key: "notifyServerConnectionFailed" },
                { label: "Scraping Failures", key: "notifyScrapingFailures" },
              ].map((item) => (
                <div key={item.key} className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-700 dark:text-slate-300">{item.label}</span>
                  <ToggleSwitch checked={(settings as any)[item.key]} onChange={() => {}} />
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === "admin-profile" && (
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-4">
            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-4">Admin Profile</h2>
            <div className="space-y-4">
              <div>
          ...
          {adminProfileSaveState === 'saved' && <Alert type="success" message="Profile saved." />}
          {adminProfileError && <Alert type="error" message={adminProfileError} />}
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors"
            >
              <Save size={16} /> {adminProfileSaveState === "saving" ? "Saving..." : "Save Changes"}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleChangePasswordSubmit} className="p-6 space-y-4">
              {passwordModalError && <Alert type="error" message={passwordModalError} />}
              {passwordModalSaveState === 'saved' && <Alert type="success" message="Password updated!" />}
              <input type="password" placeholder="Current Password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm" required />
              <input type="password" placeholder="New Password" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm" required />
              <input type="password" placeholder="Confirm New Password" value={confirmNewPassword} onChange={e => setConfirmNewPassword(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm" required />
              <div className="flex justify-end gap-3 pt-4">
                <button type="button" onClick={() => setIsPasswordModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-600">Cancel</button>
                <button type="submit" disabled={passwordModalSaveState === 'saving'} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg">
                  {passwordModalSaveState === 'saving' ? 'Saving...' : 'Update'}
                </button>
              </div>
            </form>
          </div>
