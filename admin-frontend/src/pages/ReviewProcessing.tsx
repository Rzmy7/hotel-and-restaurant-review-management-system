...
export const ReviewProcessing: React.FC = () => {
  ...
  const [geminiConfig, setGeminiConfig] =
    useState<GeminiApiKeyConfig>(defaultGeminiConfig);
  const [geminiApiKeyInput, setGeminiApiKeyInput] = useState("");
  const [geminiSaveState, setGeminiSaveState] = useState<
    "idle" | "saving" | "saved" | "error"
  >("idle");
  const [geminiSaveMessage, setGeminiSaveMessage] = useState<string | null>(
    null,
  );
  const [geminiTestState, setGeminiTestState] = useState<
    "idle" | "testing" | "success" | "error"
  >("idle");
  const [geminiTestMessage, setGeminiTestMessage] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const loadData = async (isRefresh = false) => {
      try {
        ...
        const [statsResult, jobsResult, geminiResult] =
        ...
        setError(
          "Failed to load review processing data. Check admin-backend connectivity.",
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const handleTestGeminiKey = async () => {
    ...
    try {
      ...
      setGeminiTestMessage(
        err instanceof Error ? err.message : "Failed to test Gemini API key.",
      );
    }
  };
    ...
    <div className="space-y-6 pt-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 mt-1">
                The system detected that the Gemini API has reached its usage
                limit or quota. Review processing has been automatically paused
                to prevent further errors. Please update your API key or quota
                settings, then click below to restart.
              </p>
            </div>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-5">
          ...
          <div className="text-2xl font-bold text-gray-900 dark:text-white">
            {formatNumber(stats.reviewsProcessed)}
          </div>
          <div className="text-xs text-green-600">
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
              Configure the Gemini API key used for AI-powered review processing
              and analysis.
            </p>
          </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-1.5">
              ...
              . This key is used for sentiment analysis, categorization, and
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {geminiTestMessage && (
              <span
                className={`text-sm ${geminiTestState === "success" ? "text-green-600" : "text-red-600"}`}
              >
                {geminiTestMessage}
              </span>
            )}
            {geminiSaveMessage && (
              <span
                className={`text-sm ${geminiSaveState === "saved" ? "text-green-600" : "text-red-600"}`}
              >
                {geminiSaveMessage}
              </span>
            )}
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">
              Real-time monitoring of all active and recent review processing
              jobs.
            </p>
          </div>
            ...
            <tbody>
              {paginatedJobs.map((job) => (
                ...
                >
                  ...
                  <td className="py-4 px-4 text-sm text-gray-900 dark:text-white">
                    {job.reviewsProcessed !== null
                      ? job.reviewsProcessed
                      : "--"}
                    {job.totalReviews !== null &&
                      job.reviewsProcessed !== null && (
