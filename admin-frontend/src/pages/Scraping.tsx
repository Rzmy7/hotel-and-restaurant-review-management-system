...
import {
  ...
  fetchScrapingJobs,
  createScrapingPlatform,
  deleteScrapingPlatform,
  fetchScrapingPlatformDetails,
  updateScrapingPlatform,
  uploadPlatformScript,
  toggleScrapingPlatform,
  stopScrapingJob,
...
export const Scraping: React.FC = () => {
  ...
  const [editPlatformError, setEditPlatformError] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const loadData = async (isRefresh = false) => {
      try {
        ...
        const [statsResult, platformsResult, jobsResult] =
        ...
        if (platformsResult.status === "rejected") {
          errors.push(
            `Platforms: ${platformsResult.reason instanceof Error ? platformsResult.reason.message : "failed to load"}`,
          );
        }
        if (
          ...
          errors.push(
            "Scraping service is unreachable (stats and jobs unavailable).",
          );
        }
        ...
        setError(
          "Failed to load scraping data. Check admin-backend connectivity.",
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const togglePlatform = async (id: string) => {
      ...
      prev.map((p) =>
        p.id === id
          ? {
              ...p,
              enabled: !p.enabled,
              status: p.enabled ? "maintenance" : "active",
            }
          : p,
      ),
    );
    try {
      ...
      setPlatforms((prev) =>
        prev.map((p) =>
          p.id === id
            ? { ...p, enabled: updated.enabled, status: updated.status }
            : p,
        ),
      );
    } catch (err) {
        ...
        prev.map((p) =>
          p.id === id
            ? {
                ...p,
                enabled: !p.enabled,
                status: p.enabled ? "active" : "maintenance",
              }
            : p,
        ),
      );
      ...
      setError(
        err instanceof Error ? err.message : "Failed to toggle platform.",
      );
    }
  };
  ...
  const openEditPlatformModal = async (platform: ScrapingPlatform) => {
    ...
    setEditPlatformForm({
      ...
      attributes:
        Array.isArray(platform.attributes) && platform.attributes.length > 0
    ...
    try {
      ...
      setEditPlatformError(
        err instanceof Error ? err.message : "Failed to load platform details.",
      );
    } finally {
      setEditPlatformLoading(false);
    }
        ...
        try {
          ...
          setEditPlatformError(
            uploadErr instanceof Error
              ? uploadErr.message
              : "Platform updated, but scraping file upload failed.",
          );
      ...
      if (addPlatformFile) {
        try {
          ...
          setAddPlatformError(
            uploadErr instanceof Error
              ? uploadErr.message
              : "Platform created, but scraping file upload failed.",
          );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-5">
          ...
          <div className="text-xs text-green-600">
            +{stats?.activeJobsChange} since last hour
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-5">
          ...
          <div className="text-2xl font-bold text-gray-900 dark:text-white">
            {formatNumber(stats?.reviewsIngested || 0)}
          </div>
          <div className="text-xs text-green-600">
            +{stats?.reviewsChange}% vs last week
          </div>
        </div>
      </div>
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">
              Configure scraper status and frequency settings for supported
              platforms.
            </p>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md rounded-xl bg-white dark:bg-slate-800 shadow-xl mx-4">
            <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-slate-700">
              ...
              <button
                onClick={() => setIsAddPlatformOpen(false)}
                className="p-1 rounded hover:bg-gray-100 dark:hover:bg-slate-600 text-gray-500 dark:text-slate-400"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleCreatePlatform} className="p-5 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">
                  Optional, but useful for organization-source linking.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400">
                  Examples: INT, BIGINT, BIT, DATE, DATETIME, DECIMAL(10,2),
              ...
              <div>
                ...
                <label className="flex flex-col items-center justify-center w-full h-20 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-500 dark:text-slate-400 hover:border-blue-400 hover:text-blue-500 cursor-pointer transition-colors">
                  <input
                    ...
                    className="hidden"
                    onChange={(e) =>
                      setAddPlatformFile(e.target.files?.[0] ?? null)
                    }
                  />
                  {addPlatformFile ? (
                    <span className="flex items-center gap-1.5 text-xs text-green-600 font-medium">
                      <Upload size={13} />
                      {addPlatformFile.name}
                    </span>
                  ) : (
                    <span className="flex items-center gap-1.5">
                      <Upload size={14} />
                      Click to upload scraping file
                    </span>
                  )}
                </label>
              </div>
      ...
      {isEditPlatformOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div
            className="absolute inset-0 bg-black/50"
            onClick={() =>
              !editPlatformSubmitting && setIsEditPlatformOpen(false)
            }
          />
                ...
                <div>
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-20 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-500 dark:text-slate-400 hover:border-blue-400 hover:text-blue-500 cursor-pointer transition-colors">
                    <input
                      ...
                      className="hidden"
                      onChange={(e) =>
                        setEditPlatformFile(e.target.files?.[0] ?? null)
                      }
                    />
                    {editPlatformFile ? (
                      <span className="flex items-center gap-1.5 text-xs text-green-600 font-medium">
                        <Upload size={13} />
                        {editPlatformFile.name}
                      </span>
                    ) : (
                      <span className="flex items-center gap-1.5">
                        <Upload size={14} />
                        Click to upload new scraping file
                      </span>
                    )}
                  </label>
                </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">
              Real-time monitoring of all active and recent scraping jobs.
            </p>
          </div>
          <div className="flex items-center gap-3">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-slate-700">
                <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">
                  Job ID
                </th>
                <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">
                  Platform
            ...
            <tbody>
              {paginatedJobs.map((job) => (
                ...
                >
                  <td className="py-4 px-4 text-sm font-mono text-gray-500 dark:text-slate-400">
                    {job.jobId}
                  </td>
                  ...
                  <td className="py-4 px-4">
                    <span
                      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${getStatusBadgeClass(job.status)}`}
                    >
                      {job.status === "Running" && (
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-2">
                      ...
                      {job.status === "Failed" && (
                        <>
                          <button className="text-xs font-semibold text-red-600 hover:text-red-700 uppercase">
                            Retry
                          </button>
        ...
        <div className="flex items-center justify-between pt-4 mt-4 border-t border-gray-100 dark:border-slate-700">
          <span className="text-sm text-gray-500 dark:text-slate-400">
            Showing{" "}
            {filteredJobs.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}{" "}
