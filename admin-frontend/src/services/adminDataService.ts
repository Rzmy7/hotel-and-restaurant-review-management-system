...
export const fetchOrgSources = (orgId: string): Promise<OrgSource[]> => {
  return apiClient.get<OrgSource[]>(
    `/admin/organizations/${encodeURIComponent(orgId)}/sources`,
  );
};
...
export const updateOrgSources = (
  ...
  return apiClient.put<OrgSource[]>(
    `/admin/organizations/${encodeURIComponent(orgId)}/sources`,
    { sources },
  );
};
...
export const triggerPendingEmbeddings = (): Promise<{
  triggered_sources_count: number;
  message: string;
}> => {
  return apiClient.post<{ triggered_sources_count: number; message: string }>(
    "/admin/embeddings/trigger-pending",
    {},
  );
};
