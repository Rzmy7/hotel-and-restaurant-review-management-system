...
const getServerUrls = () => {
  return {
    mainBackend:
      localStorage.getItem("mainBackendUrl") || DEFAULT_MAIN_BACKEND_URL,
    scraping:
      localStorage.getItem("scrapingBackendUrl") || DEFAULT_SCRAPING_URL,
    embedding:
      localStorage.getItem("embeddingServiceUrl") || DEFAULT_EMBEDDING_URL,
...
export const fetchServerStatuses = async (): Promise<ServerStatus[]> => {
  ...
  const servers: Array<{
    ...
    // Main Backend card reads admin-backend server stats endpoint (same server context).
    {
      id: "1",
      name: "Main Backend",
      url: urls.mainBackend,
      healthPath: "/admin/monitoring/main-backend-status",
      icon: Server,
    },
    {
      id: "2",
      ...
      url: urls.scraping,
      healthPath: "/api/system/admin-health",
      icon: Search,
    },
    {
      id: "3",
    ...
    },
    {
      ...
      url: urls.mainBackend,
      healthPath: "/admin/monitoring/frontend-status",
      icon: Globe,
    },
  ];
 ...
 */
export const fetchSingleServerStatus = async (
  serverName: "mainBackend" | "scraping" | "embedding" | "frontend",
