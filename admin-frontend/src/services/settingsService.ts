...
export const emitSystemTimezoneUpdated = (timezone: string): void => {
  window.dispatchEvent(
    new CustomEvent<string>(SYSTEM_TIMEZONE_UPDATED_EVENT, {
      detail: timezone,
    }),
  );
};
...
export const settingsService = {
  async getGeneralSettings(): Promise<GeneralSettings> {
    const settings = await apiClient.get<GeneralSettings>(
  ...
  async updateGeneralSettings(
    ...
    const settings = await apiClient.patch<GeneralSettings>(
      "/admin/settings/general",
      payload,
    );
    applySystemTimezone(settings.timezone);
  ...
  async getReplyGenerationSettings(): Promise<ReplyGenerationSettings> {
    return apiClient.get<ReplyGenerationSettings>(
      "/admin/settings/reply-generation",
    );
  },

  async updateReplyGenerationSettings(
    payload: ReplyGenerationSettings,
  ): Promise<ReplyGenerationSettings> {
    ...
    return apiClient.patch<ReplyGenerationSettings>(
      "/admin/settings/reply-generation",
      requestPayload,
    );
  },

  async testReplyGenerationApiKey(
    payload: ReplyGenerationApiTestPayload,
  ): Promise<ReplyGenerationApiTestResponse> {
    return apiClient.post<ReplyGenerationApiTestResponse>(
      "/admin/settings/reply-generation/test",
      payload,
    );
  },
  ...
  async updateAdminProfile(payload: AdminProfile): Promise<AdminProfile> {
    return apiClient.patch<AdminProfile>(
  ...
  async changeAdminPassword(
    ...
    return apiClient.patch<AdminPasswordChangeResponse>(
