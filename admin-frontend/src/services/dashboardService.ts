...
export const fetchUsageData = async (): Promise<ChartDataPoint[]> => {
    const response = await apiClient.get<{ trendData: any[] }>('/dashboard/usage');
...
export const fetchReviewData = async (): Promise<ChartDataPoint[]> => {
    const response = await apiClient.get<{ reviews: any[] }>('/dashboard/reviews');
...
export const fetchSystemAlerts = async (): Promise<SystemAlert[]> => {
    const response = await apiClient.get<{ alerts: SystemAlert[] }>('/dashboard/alerts');
    return response.alerts || [];
};

export const fetchRecentActivity = async (): Promise<RecentActivity[]> => {
    const response = await apiClient.get<{ activities: RecentActivity[] }>('/dashboard/activities');
