...
const normalizeTableAttributes = (
  attributes: RawScrapingPlatformDetailsResponse["attributes"],
) => {
...
export const fetchScrapingPlatforms = (): Promise<ScrapingPlatform[]> => {
  return apiClient.get<ScrapingPlatform[]>(
    "/admin/monitoring/scraping/platforms",
  );
};
...
export const createScrapingPlatform = (
  payload: CreateScrapingPlatformPayload,
): Promise<ScrapingPlatform> => {
  return apiClient.post<ScrapingPlatform>(
    "/admin/monitoring/scraping/platforms",
    payload,
  );
};

export const fetchScrapingPlatformDetails = async (
  platformId: string,
): Promise<ScrapingPlatformDetailsResponse> => {
  const raw = await apiClient.get<RawScrapingPlatformDetailsResponse>(
    `/admin/monitoring/scraping/platforms/${encodeURIComponent(platformId)}`,
  );
  return {
...
export const updateScrapingPlatform = (
  platformId: string,
  payload: UpdateScrapingPlatformPayload,
): Promise<ScrapingPlatform> => {
  return apiClient.put<ScrapingPlatform>(
    `/admin/monitoring/scraping/platforms/${encodeURIComponent(platformId)}`,
    payload,
  );
};

export const deleteScrapingPlatform = (
  platformId: string,
): Promise<{ status: string; id: string; name: string }> => {
  return apiClient.delete<{ status: string; id: string; name: string }>(
    `/admin/monitoring/scraping/platforms/${encodeURIComponent(platformId)}`,
  );
};

export const toggleScrapingPlatform = (
  platformId: string,
): Promise<{
  id: string;
  name: string;
  ...
  return apiClient.patch<{
    ...
    `/admin/monitoring/scraping/platforms/${encodeURIComponent(platformId)}/toggle`,
    {},
  );
};

export const stopScrapingJob = (
  jobId: string,
): Promise<{ status: string; job_id: string }> => {
  return apiClient.post<{ status: string; job_id: string }>(
    `/admin/monitoring/scraping/jobs/${encodeURIComponent(jobId)}/cancel`,
    {},
  );
};
