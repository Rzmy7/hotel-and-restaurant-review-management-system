...
export const maintenanceService = {
  async getStatus(): Promise<MaintenanceStatusResponse> {
    return apiClient.get<MaintenanceStatusResponse>(
  ...
  async setStatus(
    ...
    return apiClient.patch<MaintenanceUpdateResponse>(
      "/admin/maintenance/status",
      { maintenanceMode },
    );
  },
...
export const emitMaintenanceModeUpdated = (maintenanceMode: boolean): void => {
  window.dispatchEvent(
    new CustomEvent<boolean>(MAINTENANCE_MODE_EVENT, {
      detail: maintenanceMode,
    }),
  );
};

export const onMaintenanceModeUpdated = (
  handler: (maintenanceMode: boolean) => void,
): (() => void) => {
