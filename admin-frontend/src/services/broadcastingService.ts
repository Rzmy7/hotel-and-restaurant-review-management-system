...
import type {
  BroadcastRecord,
  ComposeForm,
} from "../components/Broadcasting/types";
...
export const broadcastingService = {
  ...
  async sendBroadcast(
    form: ComposeForm,
  ): Promise<{ success: boolean; broadcastId: string; message: string }> {
    return apiClient.post<{
      success: boolean;
      broadcastId: string;
      message: string;
    }>("/admin/broadcasting/send", form);
  },

  async getBroadcastDetail(broadcastId: string): Promise<BroadcastRecord> {
    return apiClient.get<BroadcastRecord>(
      `/admin/broadcasting/${encodeURIComponent(broadcastId)}`,
    );
  },
  ...
  async getEstimatedRecipients(
    ...
    const result = await apiClient.get<{ count: number }>(
      `/admin/broadcasting/estimate-recipients?${params}`,
    );
    return result.count;
  },
  ...
  async resendBroadcast(
    ...
    return apiClient.post<{ success: boolean; message: string }>(
      `/admin/broadcasting/${encodeURIComponent(broadcastId)}/resend`,
      {},
    );
  },
  ...
  async cancelBroadcast(
    ...
    return apiClient.post<{ success: boolean; message: string }>(
      `/admin/broadcasting/${encodeURIComponent(broadcastId)}/cancel`,
      {},
    );
  },
};
