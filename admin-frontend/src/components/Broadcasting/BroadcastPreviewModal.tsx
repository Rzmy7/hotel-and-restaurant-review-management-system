...
export const BroadcastPreviewModal: React.FC<PreviewModalProps> = ({
  form,
  estimatedCount,
  timezone,
  onClose,
  ...
  const audienceOpt = AUDIENCE_OPTIONS.find(
    (a) => a.value === form.audienceType,
  )!;
  const subLabel =
    audienceOpt.subOptions?.find((s) => s.value === form.audienceValue)
      ?.label ?? "";
  const audienceLabel =
    form.audienceType === "all"
      ? "All Users"
      : `${audienceOpt.label}: ${subLabel}`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg mx-4">
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Message type badge */}
          <div
            className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${mt.bg} ${mt.color}`}
          >
            {mt.icon}
            {mt.label}
          </div>
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-800 dark:text-slate-100">
                {form.scheduleType === "now"
                  ? "Send immediately"
                  : formatDateTime(form.scheduledAt, timezone)}
              </p>
            </div>
          </div>
        </div>
