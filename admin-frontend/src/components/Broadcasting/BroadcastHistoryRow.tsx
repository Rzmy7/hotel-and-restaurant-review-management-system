...
export const BroadcastHistoryRow: React.FC<HistoryRowProps> = ({
  record,
  timezone,
  onViewDetail,
}) => {
  ...
  return (
    <div className="flex items-center gap-4 px-4 py-3.5 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition-colors border-b border-gray-100 dark:border-slate-700 last:border-0">
      {/* Type indicator */}
      <div
        className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center border ${mt.bg} ${mt.color}`}
      >
        {mt.icon}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-gray-200">·</span>
          <span className="text-xs text-gray-500 dark:text-slate-400">
            {record.recipientCount.toLocaleString()} recipients
          </span>
        </div>
      </div>
