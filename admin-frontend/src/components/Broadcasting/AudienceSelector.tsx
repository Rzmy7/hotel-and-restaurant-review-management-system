        ...
        {AUDIENCE_OPTIONS.map((opt) => (
          <div key={opt.value}>
            <label
              ...
              style={{
                borderColor: audienceType === opt.value ? "#2563eb" : "#e5e7eb",
              }}
            >
              <input
                type="radio"
            ...
            {audienceType === opt.value && opt.subOptions && (
              <div className="ml-8 mt-2 space-y-1.5">
                {opt.subOptions.map((sub) => (
                  <label
                    key={sub.value}
                    className="flex items-center gap-2 p-2 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 rounded cursor-pointer"
                  >
                    <input
                      type="radio"
                      name={`${opt.value}_sub`}
