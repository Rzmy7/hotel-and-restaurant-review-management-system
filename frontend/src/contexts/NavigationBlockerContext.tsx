...
export const NavigationBlockerProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  const [isDirty, setIsDirty] = useState(false);
  const [blockHandler, setBlockHandler] = useState<
    ((targetPath: string) => void) | null
  >(null);
  ...
  const registerBlockHandler = useCallback(
    (handler: (targetPath: string) => void) => {
