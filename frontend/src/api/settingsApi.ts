...
const mapOrganizationToHotelInfo = (
  org: UserOrganization,
): SettingsData["hotelInfo"] => ({
...
const fetchUserOrganizations = async (): Promise<UserOrganization[]> => {
  const response = await settingsAxios.get<UserOrganization[]>(
    toApiPath("/user/organizations"),
  );
  return Array.isArray(response.data) ? response.data : [];
};

const resolveOrganizationTypeId = async (
  propertyType: string,
  ...
  try {
    const response = await settingsAxios.get<OrganizationType[]>(
      toApiPath("/organization-types"),
    );
    const matched = response.data.find(
      (type) => type.type_name?.trim().toLowerCase() === normalizedTarget,
    );
...
const patchHotelInfoToBackend = async (
  hotelInfoUpdates: Partial<SettingsData["hotelInfo"]>,
) => {
  ...
  if (hotelInfoUpdates.hotelName !== undefined) {
    payload.organization_name =
      normalizeEmpty(hotelInfoUpdates.hotelName) ?? null;
  }
  ...
  if (hotelInfoUpdates.primaryEmail !== undefined) {
    payload.primary_email =
      normalizeEmpty(hotelInfoUpdates.primaryEmail) ?? null;
  }
  ...
  if (hotelInfoUpdates.propertyType !== undefined) {
    const organizationTypeId = await resolveOrganizationTypeId(
      hotelInfoUpdates.propertyType,
    );
...
export const settingsApi = {
  fetchSettings: async (): Promise<SettingsData> => {
    ...
    try {
      const profileResponse = await settingsAxios.get<UserProfile>(
        toApiPath("/users/me"),
      );
      currentSettings = {
        ...currentSettings,
...
export const settingsApi = {
  ...
  fetchOrganizationRules: async (): Promise<
    Array<{
      rule_id: string;
      rule_text: string;
      rule_order: number;
