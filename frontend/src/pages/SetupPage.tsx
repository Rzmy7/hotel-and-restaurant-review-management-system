...
const restoreSnapshotOrganizations = () => {
  const snapshotCurrentOrganization = localStorage.getItem(
    SETUP_SNAPSHOT_CURRENT_ORG_KEY,
  );
...
const SetupPage = () => {
  ...
  const checkOrganizationLimit = async (): Promise<boolean> => {
    ...
    try {
      ...
      const orgFeature = usage.features.find(
        (feature) => feature.key === "organizations",
      );
      const organizationsCount = organizations.length;

      if (
        orgFeature?.limit !== null &&
        orgFeature?.limit !== undefined &&
        organizationsCount >= orgFeature.limit
      ) {
        setLimitError(
          "Organization limit reached for your current plan. Upgrade your plan to add more organizations.",
        );
        return true;
      }
      ...
      try {
        ...
        if (draftStr) {
          ...
          if (draft.organization?.name)
            setOrganizationName(draft.organization.name);
  ...
  const handleSkip = async () => {
    const hasPendingSetupOrganization = Boolean(
      localStorage.getItem(SETUP_PENDING_ORG_ID_KEY),
    );
    const hasSnapshot =
      localStorage.getItem(SETUP_SNAPSHOT_CURRENT_ORG_KEY) !== null;
    ...
    if (hasPendingSetupOrganization || hasSnapshot) {
      ...
      const committedOrgIds = parseJsonArray(
        localStorage.getItem("organization_ids"),
      );
    ...
    const existingOrgIds = parseJsonArray(
      localStorage.getItem("organization_ids"),
    );
    const hasExistingOrganizations =
      Array.isArray(existingOrgIds) && existingOrgIds.length > 0;
    ...
    >
      ...
      <div className="mb-6">
        <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider block mb-2">
          ...
          <span className="text-slate-400 dark:text-slate-500 font-medium normal-case tracking-normal ml-2">
            (Optional)
          </span>
        </label>
        <input
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1.5">
          Upload your property rules. AI will extract individual rules for
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div
                      className={`text-[15px] font-black uppercase tracking-tight mb-1 transition-colors ${isSelected ? "text-blue-600" : "text-slate-900 dark:text-white"}`}
                    >
                      {type.type_name}
                    </div>
