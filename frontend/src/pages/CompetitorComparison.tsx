...
const CompetitorComparison = () => {
  ...
  const loadData = useCallback(async () => {
    if (!competitorId) {
      setError(
        "No competitor ID specified. Go back and click Compare on a competitor.",
      );
      setLoading(false);
  ...
  useEffect(() => {
    if (organizationId) {
      fetchCompetitors(organizationId)
        .then((res) => setTrackedList(res.tracked))
        .catch(() => {});
    }
  }, [organizationId]);
  ...
  const renderKpi = (
    ...
    return (
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-6 flex flex-col">
        <h3 className="text-[11px] font-bold text-gray-400 dark:text-slate-400 uppercase tracking-widest mb-4">
          {title}
        </h3>
        <div className="space-y-3">
          <div className="flex justify-between items-center text-sm">
            <span
              className="text-[#4e80ee] font-medium truncate max-w-[55%]"
              title={myName}
            >
              {myName}
            </span>
            <span className="font-bold text-gray-900 dark:text-white text-[17px]">
              {fmt(kpi.myHotel)}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span
              className="text-green-500 font-medium truncate max-w-[55%]"
              title={compName}
            >
              {compName}
            </span>
  ...
  return (
    <div className="min-h-full bg-gray-50 dark:bg-slate-900 flex flex-col font-sans">
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-100 dark:border-slate-700/80 sticky top-0 z-[40] px-8 py-5 flex items-center justify-between transition-all duration-300">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-gray-400 dark:text-slate-400 font-bold uppercase tracking-wider">
            {myName}{" "}
            <span className="text-gray-300 dark:text-slate-600 mx-1">vs</span>{" "}
            {compName}
          </p>
        </div>
        ...
        <div ref={dropdownRef} className="relative">
          ...
          >
            ...
            <ChevronDown
              size={15}
              className={`text-gray-400 dark:text-slate-500 flex-shrink-0 transition-transform ${dropdownOpen ? "rotate-180" : ""}`}
            />
          </button>
      ...
      <main className="w-full px-8 py-8 flex-1 max-w-[1400px] mx-auto space-y-8">
        <Link
          to="/competitors"
          className="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white font-medium transition-colors"
        >
          <ArrowLeft size={16} />
          Back to Competitors
        </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {renderKpi(
            "Average Rating",
            comparison.kpis.avgRating,
            (v) => v.toFixed(1),
            `${compName} is ahead`,
          )}
          {renderKpi(
            "Review Count",
            comparison.kpis.reviewCount,
            (v) => v.toLocaleString(),
            `Fewer reviews than ${compName}`,
          )}
          {renderKpi(
            "Positive %",
            comparison.kpis.positivePercent,
            (v) => `${v}%`,
            "Lower positive sentiment",
          )}
          {renderKpi(
            "Negative %",
            comparison.kpis.negativePercent,
            (v) => `${v}%`,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-6">
            <h3 className="text-[11px] font-bold text-gray-400 dark:text-slate-400 uppercase tracking-widest mb-6">
              Aspect Comparison
            </h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Line
                    ...
                    stroke="#4e80ee"
                    strokeWidth={2}
                    dot={{ r: 5, fill: "#4e80ee", strokeWidth: 0 }}
                    activeDot={{ r: 7 }}
                  />
                  <Line
                    type="monotone"
        ...
        <div className="bg-[#f5f8ff] dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800/50 p-6 flex flex-col gap-4">
          ...
          {insightsLoading ? (
            ...
            <>
              <div className="flex flex-wrap items-center gap-3">
                {insights.strengths?.slice(0, 2).map((s, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-[#dcfce7] dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-full text-sm font-semibold border border-green-200 dark:border-green-800/30 shadow-sm"
                  >
                    <CheckCircle size={16} />
                    {s}
                  </div>
                ...
                {insights.weaknesses?.slice(0, 1).map((w, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 rounded-full text-sm font-semibold border border-yellow-200 dark:border-yellow-800/30 shadow-sm"
                  >
            ...
            <p className="text-sm text-gray-400 dark:text-slate-500">
              Unable to generate insights — Gemini API may be unavailable.
            </p>
          )}
        </div>
      </main>
