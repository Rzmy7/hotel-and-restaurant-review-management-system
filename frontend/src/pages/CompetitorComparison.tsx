import { ArrowLeft, Bell, AlertTriangle, Lightbulb, CheckCircle, Loader2 } from 'lucide-react';
...
const CompetitorComparison = () => {
    ...
    const competitorName = comparison?.competitor?.name ?? 'Competitor';
    ...
    const renderKpi = (title: string, kpi: KpiData, formatFn?: (v: number) => string, gapLabel?: string) => {
        ...
        return (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-6 flex flex-col">
                <h3 className="text-xs font-bold text-gray-400 dark:text-slate-400 uppercase tracking-wider mb-4">{title}</h3>
                <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center text-sm">
                        <span className="text-green-500 dark:text-green-400 font-medium">{competitorName}</span>
    ...
    if (error) {
        return (
            <div className="min-h-full bg-gray-50 dark:bg-slate-900 flex flex-col items-center justify-center gap-4">
                <p className="text-red-500">{error}</p>
                <Link to="/competitors" className="text-blue-500 hover:underline text-sm">Back to Competitors</Link>
            </div>
        );
    }
            ...
            <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-100 dark:border-slate-700/80 sticky top-0 z-[40] px-8 py-5 flex items-center justify-between transition-all duration-300">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-400 dark:text-slate-400">vs {competitorName}</p>
                    </div>
                </div>
            ...
            <main className="w-full px-8 py-8 flex-1 max-w-[1400px] mx-auto space-y-8">
                ...
                {comparison?.kpis && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        {renderKpi('Review Count', comparison.kpis.reviewCount, (v) => v.toLocaleString(), 'Fewer reviews')}
                        ...
                        <div className="h-[280px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={aspectData}>
                                    ...
                                    <Radar name={competitorName} dataKey="competitor" stroke="#22c55e" fill="#22c55e" fillOpacity={0.1} strokeWidth={2.5} dot={{ r: 4, fill: '#22c55e' }} activeDot={{ r: 6 }} />
                        ...
                        <div className="h-[280px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={trendData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                                    ...
                                    <Line type="monotone" dataKey="competitor" name={competitorName} stroke="#22c55e" strokeWidth={2} dot={{ r: 5, fill: '#22c55e', strokeWidth: 0 }} activeDot={{ r: 7 }} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                        ...
                        <div className="h-[280px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={sentimentData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                                    ...
                                    <Legend iconType="circle" wrapperStyle={{ fontSize: '13px', paddingTop: '20px' }} />
                                    <Bar dataKey="competitor" name={competitorName} fill="#22c55e" radius={[2, 2, 0, 0]} />
                        ...
                        <>
                            <div className="flex flex-wrap items-center gap-3">
                                {insights.tags?.map((tag, i) => (
                                    <div key={i} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold border shadow-sm ${
                                        tag.type === 'positive'
                                            ? 'bg-[#dcfce7] dark:bg-green-900/20 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800/30'
                            ...
                            {insights.strengths?.length > 0 && (
                                <div>
                                    <h4 className="text-sm font-bold text-green-700 dark:text-green-400 mb-1">Strengths</h4>
                            ...
                            {insights.weaknesses?.length > 0 && (
                                <div>
                                    <h4 className="text-sm font-bold text-red-600 dark:text-red-400 mb-1">Weaknesses</h4>
                                    <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-300 space-y-1">
                            ...
                            {insights.recommendations?.length > 0 && (
                                <div>
                                    <h4 className="text-sm font-bold text-blue-600 dark:text-blue-400 mb-1">Recommendations</h4>
                                    <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-300 space-y-1">
