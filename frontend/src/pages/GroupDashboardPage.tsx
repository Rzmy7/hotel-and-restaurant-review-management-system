...
const StatCard: React.FC<{
  ...
  <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-5 hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)] transition-all duration-200">
    <div className="flex items-center justify-between mb-3">
      ...
      <div
        className={`w-9 h-9 rounded-xl flex items-center justify-center ${iconBg} ${iconFg}`}
      >
        {icon}
      </div>
    </div>
        ...
        {!selected && query.trim().length > 0 && (
          <div className="border border-gray-200 dark:border-slate-600 rounded-xl overflow-hidden mb-4 max-h-52 overflow-y-auto">
            {searching ? (
              ...
              <p className="text-center text-sm text-gray-400 dark:text-slate-500 py-5">
              ...
              results.map((org) => (
                <button
                  ...
                  onClick={() => {
                    setSelected(org);
                    setQuery(org.organization_name);
                    setResults([]);
                  }}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-2xl w-full max-w-sm p-6 text-center">
      ...
      <p className="text-sm text-gray-500 dark:text-slate-400 mb-6">
        ...
        action cannot be undone. All members and invites will be removed.
      </p>
      <div className="flex gap-3">
        <button
          onClick={onClose}
  ...
  return (
    <div className="space-y-6">
        ...
        {canSeeAnalytics && analytics ? (
          ...
          <>
            <StatCard
              ...
              value="—"
              icon={<Star size={16} />}
              sub={canSeeAnalytics ? "Loading…" : "Restricted"}
            />
            <StatCard
              label="Avg Rating"
              value="—"
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-5">
        <h3 className="text-sm font-black text-gray-700 dark:text-slate-300 uppercase tracking-wider mb-3">
          Group Info
        </h3>
      ...
      {analytics && analytics.member_orgs.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-5">
          <h3 className="text-[11px] font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-4">
              ...
              >
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs font-bold text-gray-700 dark:text-slate-300">
                    {org.review_count} reviews
                  </p>
                  {org.avg_rating && (
                    <p className="text-xs text-gray-400">{org.avg_rating}★</p>
                  )}
                </div>
              </div>
            ))}
...
const MembersTab: React.FC<{
  ...
  const handleRemove = async (organizationId: string, orgName: string) => {
    ...
    try {
      ...
      setMembers((prev) =>
        prev.filter((m) => m.organization_id !== organizationId),
      );
      onMemberRemoved();
    } catch (err: any) {
    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">
          Invite organizations to join this group
        </p>
      </div>
    );
      ...
      <div className="px-6 py-4 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-700 dark:text-slate-300">
            {members.length} member organization
            {members.length !== 1 ? "s" : ""}
          </span>
        </div>
      </div>
      <div className="divide-y divide-gray-100 dark:divide-slate-700">
        {members.map((m) => {
          ...
          return (
            <div
              key={m.user_id}
              className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50/50 dark:hover:bg-slate-700/30 transition-colors"
            >
...
const InvitesTab: React.FC<{
  ...
  const inviteUrl = linkToken
    ? `${frontendBase}/groups/join/${linkToken}`
    : null;
  ...
  const handleRevokeLink = async () => {
    if (
      !confirm(
        "Revoke invite link? Anyone who had it will no longer be able to join.",
      )
    )
      return;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-[11px] font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest">
            Invite Link
          </h3>
        </div>
        ...
        {inviteUrl ? (
          <div className="space-y-3">
            <div className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-slate-700 rounded-xl border border-gray-200 dark:border-slate-600">
              <span className="text-xs text-gray-600 dark:text-slate-300 truncate flex-1 font-mono">
                {inviteUrl}
              </span>
              <button
                onClick={handleCopy}
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">
                Search and invite organizations to join this group
              </p>
            </div>
            <button
            ...
            <div className="space-y-2">
              {invites.map((inv) => {
                ...
                return (
                  <div
                    key={inv.invite_id}
                    className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-slate-700/50 border border-gray-100 dark:border-slate-700"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {inv.message && (
                        <p className="text-xs text-gray-500 dark:text-slate-400 italic mt-0.5">
                          "{inv.message}"
                        </p>
                      )}
                    </div>
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${statusBadge(inv.status)}`}
                    >
                      {inv.status}
                    </span>
                    {inv.status === "pending" && (
...
const AnalyticsTab: React.FC<{
    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-sm text-gray-400 dark:text-slate-500 mt-1">
          The group owner has not enabled analytics visibility for members.
        </p>
      </div>
    );
  ...
  const sentimentData = [
    ...
    {
      name: "Negative",
      value: analytics.negative_count,
      color: SENTIMENT_COLORS.negative,
    },
    {
      name: "Neutral",
      value: analytics.neutral_count,
  ...
  const ratingMap = Object.fromEntries(
    analytics.rating_distribution.map((r) => [r.star, r.count]),
  );
  const ratingData = allStars.map((s) => ({
    star: `${s}★`,
    count: ratingMap[s] || 0,
  }));

  const acceptanceRate =
    analytics.invite_stats.total_sent > 0
      ? Math.round(
          (analytics.invite_stats.accepted_count /
            analytics.invite_stats.total_sent) *
            100,
        )
      : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={analytics.positive_count.toLocaleString()}
          icon={<Check size={16} />}
        />
        <StatCard
      ...
      {analytics.reviews_over_time.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart
              data={analytics.reviews_over_time}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        {sentimentData.length > 0 && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-5">
            <h3 className="text-sm font-black text-gray-700 dark:text-slate-300 uppercase tracking-wider mb-4">
      ...
      {analytics.member_orgs.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 dark:border-slate-700">
            <h3 className="text-sm font-black text-gray-700 dark:text-slate-300 uppercase tracking-wider">
              Member Organizations
            </h3>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-slate-700">
              ...
              >
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold text-gray-900 dark:text-white">
                    {org.review_count.toLocaleString()}
                  </p>
                  <p className="text-xs text-gray-400 dark:text-slate-500">
                    {org.avg_rating ? `${org.avg_rating}★` : "—"}
                  </p>
                </div>
              </div>
            ))}
      ...
      {analytics.total_reviews === 0 && (
        <div className="text-center py-8 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700">
          <BarChart3
            ...
            className="text-gray-300 dark:text-slate-600 mx-auto mb-3"
          />
          <p className="font-semibold text-gray-500 dark:text-slate-400 text-sm">
            No review data yet
          </p>
          <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">
            Review analytics will appear as members add organizations with
            review sources
          </p>
        </div>
      )}
...
const SettingsTab: React.FC<{
  ...
  const ToggleRow: React.FC<{
    ...
    <div className="flex items-center gap-3 py-3 border-b border-gray-100 dark:border-slate-700 last:border-b-0">
      ...
      >
        <span
          className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform duration-200 ${value ? "translate-x-5" : "translate-x-1"}`}
        />
      </button>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-5">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-slate-600">
            ...
            >
              <span
                className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform duration-200 ${isPrivate ? "translate-x-5" : "translate-x-1"}`}
              />
            </button>
            <div>
      ...
      {/* Member permissions */}
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-5">
        <h3 className="text-[11px] font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-1">
          Member Permissions
        </h3>
        ...
        <ToggleRow
          ...
          onChange={(v) =>
            setSettings((s) => ({ ...s, can_members_invite: v }))
          }
        />
        <ToggleRow
          ...
          onChange={(v) =>
            setSettings((s) => ({ ...s, show_members_to_members: v }))
          }
        />
        <ToggleRow
          ...
          onChange={(v) =>
            setSettings((s) => ({ ...s, show_analytics_to_members: v }))
          }
        />
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-red-200 dark:border-red-900/40 shadow-sm p-5">
        <h3 className="text-[11px] font-black text-red-500 uppercase tracking-widest mb-2">
          Danger Zone
        </h3>
        <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">
          Deleting this group is permanent. All members and invite history will
          be removed.
        </p>
        <button
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <span
                className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-widest ${
                  isOwner
                    ? "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
      ...
      <main className="w-full px-8 py-8 flex-1 max-w-[1600px] mx-auto">
        ...
        {activeTab === "analytics" && (
          <AnalyticsTab
            ...
            restricted={analyticsRestricted}
          />
        )}
