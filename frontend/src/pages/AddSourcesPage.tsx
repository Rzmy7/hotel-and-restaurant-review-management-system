    ...
    >
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center gap-6">
                  ...
                  <div className="flex-1 relative group">
                    <Globe
                      className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${isConnected ? "text-blue-500" : "text-slate-400"}`}
                    />
                    <Input
                      type="text"
                      ...
                      value={url}
                      onChange={(e) =>
                        updateSourceUrl(platform.platform_id, e.target.value)
                      }
          ...
          {platforms.length === 0 && (
            <div className="text-center py-10 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
              <p className="text-slate-500 font-medium italic">
                No review platforms enabled in the system yet.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800">
        ...
        <p className="text-[13px] text-slate-500 dark:text-slate-400 font-medium">
          We are constantly adding new platforms. If you have a specific
          requirement, contact our support team.
        </p>
      </div>
    </SetupLayout>
  );
