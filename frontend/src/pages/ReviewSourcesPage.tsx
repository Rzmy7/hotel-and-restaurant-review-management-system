...
const ReviewSourcesPage = () => {
  ...
  const {
    ...
    isRefetching: isRefreshingSources,
  } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const wasRecentlyTriggered =
        lastSyncTriggeredAt && Date.now() - lastSyncTriggeredAt < 30000;
  ...
  const {
    ...
    getNextPageParam: (lastPage: SyncLog[], allPages: SyncLog[][]) =>
  ...
  const addSourceMutation = useMutation({
    mutationFn: (newSourceData: Partial<Source>) =>
      sourcesService.addSource(organizationId, newSourceData as any),
    onSuccess: () => {
  ...
  const updateSourceMutation = useMutation({
    ...
    }) => sourcesService.updateSource(id, updates),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["sources"] });
            ...
            <div className="flex bg-gray-100/50 dark:bg-slate-900/50 p-1 rounded-xl border border-gray-100 dark:border-slate-700">
              {["All", "Active", "Paused", "Error"].map((status) => (
                <button
                  ...
                  className={`px-5 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-wider transition-all duration-300 ${
                    statusFilter === status
