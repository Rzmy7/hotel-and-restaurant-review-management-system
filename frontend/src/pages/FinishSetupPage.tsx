...
const FinishSetupPage = () => {
  ...
  const showUpgradePlanAction =
    !!error && error.toLowerCase().includes("organization limit reached");
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (!draftStr) {
        throw new Error(
          "Setup data not found. Please restart the setup process.",
        );
      }
      ...
      if (organizationId) {
        ...
        const rulesFile = (window as any).__setup_rules_file as
          | File
          | undefined;
        if (rulesFile) {
          try {
            ...
            await fetch(
              `${baseUrl.replace(/\/$/, "")}/api/organizations/${organizationId}/upload-rules`,
              {
                method: "POST",
        ...
        const organizations = parseJsonArray(
  ...
  const nextSteps = [
    {
      ...
      description:
        "We are currently connecting to your sources and retrieving your latest reviews.",
      icon: Search,
    },
    {
      title: "Sentiment Analysis",
      description:
        "Our AI is processing each review to categorize feedback and determine sentiment.",
      icon: BarChart3,
    },
    {
      title: "Reporting Dashboard",
      description:
        "Your dashboard will be ready with actionable insights in approximately 5-10 minutes.",
      icon: Rocket,
    },
  ];
    ...
    >
      <div className="flex flex-col items-center text-center">
        ...
        <div className="w-full bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 text-left">
          ...
          <p className="text-[13px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
            Click the button below to finalize and head to your dashboard.
            Welcome to the ReviewMate family.
          </p>
        </div>
      </div>
    </SetupLayout>
