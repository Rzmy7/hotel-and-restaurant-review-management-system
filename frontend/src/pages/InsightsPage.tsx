...
const dataByRange: Record<string, RangeData> = {
  "7d": {
    ...
    heatmapWeeks: [[2, 4, 1, 3, 5, 8, 6]],
    aiActions: [
      {
        severity: "critical",
        title: "Wi-Fi complaints surging",
        body: "34% of negative reviews this week mention slow Wi-Fi. Upgrade network infrastructure in floors 3-5.",
      },
      {
        severity: "warning",
        title: "Food rating declining",
        body: "Restaurant rating dropped from 73% to 71%. Consider rotating menu items and improving breakfast variety.",
      },
      {
        severity: "info",
        title: "Staff performance excels",
        body: "Staff mentions are up 12% with 92% positive sentiment. Consider employee recognition program.",
      },
    ],
  },
  "30d": {
    ...
    aiActions: [
      {
        severity: "critical",
        title: "Wi-Fi is #1 complaint",
        body: "34% of negative reviews mention slow Wi-Fi. Upgrade network infrastructure — estimated 0.3-star rating impact.",
      },
      {
        severity: "critical",
        title: "Noise complaints escalating",
        body: "Room noise mentions increased 40% month-over-month. Investigate soundproofing in east-wing rooms (201-215).",
      },
      {
        severity: "warning",
        title: "Food quality declining",
        body: "Restaurant category dropped from 71% to 69%. Breakfast variety is the top complaint — consider menu refresh.",
      },
      {
        severity: "warning",
        title: "Bathroom size repeatedly cited",
        body: "22 reviews mention small bathrooms. Consider renovating compact rooms or adding storage solutions.",
      },
      {
        severity: "info",
        title: "Staff excellence recognized",
        body: "Staff score rose 5 points to 85%. Top-mentioned employees: Maria (front desk), James (concierge).",
      },
      {
        severity: "info",
        title: "Google rating climbing",
        body: "Google Reviews average increased to 4.5 — your highest-rated platform. Consider directing more guests to review on Google.",
      },
    ],
  },
  "90d": {
    ...
    aiActions: [
      {
        severity: "critical",
        title: "Wi-Fi infrastructure overhaul needed",
        body: "22% of all reviews mention connectivity. Projected 0.5-star improvement if resolved. ROI on network upgrade: 7 months.",
      },
      {
        severity: "critical",
        title: "Noise insulation project recommended",
        body: "East-wing rooms generate 3× more noise complaints. Soundproofing 15 rooms would reduce negative reviews by ~15%.",
      },
      {
        severity: "warning",
        title: "Restaurant needs menu refresh",
        body: 'Food category is your weakest at 67%. "Limited menu" appears in 39 reviews. A/B test new breakfast options.',
      },
      {
        severity: "warning",
        title: "Parking pricing competitiveness",
        body: "Competitors average $18/day vs your $28/day. 51 reviews mention expensive parking — consider loyalty discounts.",
      },
      {
        severity: "info",
        title: "Staff training ROI confirmed",
        body: "Staff score improved 7 points (76→83) since Q4 training program. Continue quarterly training investment.",
      },
      {
        severity: "info",
        title: "Response rate driving results",
        body: "Properties responding to 90%+ of reviews average 0.4 stars higher. Your rate improved from 78% to 87% this quarter.",
      },
    ],
  },
};
...
const ChangeBadge = ({ value }: { value: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 text-[13px] font-semibold px-2 py-1 rounded-md ${colors}`}
    >
      <Icon size={13} />
      {value}
    </span>
  ...
  useEffect(() => {
    ...
    const checkAccess = async () => {
      ...
      try {
        ...
        const hasInsights = usage.features.some(
          (f) => f.key === "insights" && f.enabled,
        );
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 mt-20">
          <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-gray-200 dark:border-slate-700 p-8 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-8 leading-relaxed">
              Your current plan does not include access to AI-powered insights
              and advanced analytics. Upgrade your plan to unlock these
              features.
            </p>
            <Button
              className="w-full"
      ...
      <div className="flex-1 flex flex-col gap-6 p-4 md:px-8 md:py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 min-[1000px]:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={m.label}
              className="flex items-center gap-3.5 p-[18px] bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl transition-all duration-200 hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)]"
            >
              <div
                className={`w-11 h-11 grid place-items-center ${m.bg} ${m.fg} rounded-[10px]`}
              >
                {m.icon}
              </div>
              <div className="flex-1">
                <p className="mb-1.5 text-[13px] text-gray-500 dark:text-gray-400 font-medium m-0">
                  {m.label}
                </p>
                <div className="flex items-baseline gap-2">
                  <span className="text-[26px] font-bold text-gray-800 dark:text-white">
                    {m.value}
                  </span>
        ...
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-5">
          ...
          <div className="mt-4 h-[200px] bg-gradient-to-b from-blue-500/5 dark:from-blue-500/10 to-transparent rounded-lg">
            ...
            >
              ...
              {/* Positive area */}
              <path
                d={areaPath(d.sentimentPositive)}
                fill="#3b82f6"
                opacity="0.12"
              />
              {/* Positive line */}
              <polyline
                ...
                points={linePoints(d.sentimentPositive)}
              />
              {/* Neutral line */}
              <polyline
                fill="none"
                ...
                strokeWidth="2"
                strokeDasharray="4,4"
                points={linePoints(d.sentimentNeutral)}
              />
              {/* Negative line */}
              <polyline
                fill="none"
                ...
                strokeLinecap="round"
                strokeLinejoin="round"
                points={linePoints(d.sentimentNegative)}
              />
              {/* Positive dots */}
          ...
          <div
            className={`grid text-center text-xs text-gray-400 dark:text-slate-500 mt-2`}
            style={{ gridTemplateColumns: `repeat(${sentLen}, 1fr)` }}
          >
            {d.sentimentMonths.map((m) => (
              <span key={m}>{m}</span>
            ))}
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 justify-end">
                      ...
                      <span
                        className={`text-[11px] font-semibold ${delta > 0 ? "text-emerald-500" : delta < 0 ? "text-red-500" : "text-gray-400"}`}
                      >
                        {delta > 0
                          ? `↑${delta}`
                          : delta < 0
            ...
            <div className="flex flex-wrap gap-2">
              {d.positiveKeywords.map((kw) => {
                ...
                return (
                  <span
                    key={kw.word}
                    className={`inline-flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 rounded-lg font-medium border border-emerald-100 dark:border-emerald-800/60 ${size}`}
                  >
                    {kw.word}
                    <span className="text-emerald-500 dark:text-emerald-500 font-normal text-[11px]">
                      {kw.count}
                    </span>
                  </span>
                );
            ...
            <div className="flex flex-wrap gap-2">
              {d.negativeKeywords.map((kw) => {
                ...
                return (
                  <span
                    key={kw.word}
                    className={`inline-flex items-center gap-1.5 bg-red-50 dark:bg-red-900/40 text-red-700 dark:text-red-400 rounded-lg font-medium border border-red-100 dark:border-red-800/60 ${size}`}
                  >
                    {kw.word}
                    <span className="text-red-500 dark:text-red-500 font-normal text-[11px]">
                      {kw.count}
                    </span>
                  </span>
                );
          ...
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-5">
              ...
              {[
                {
                  ...
                  value: d.responseMetrics.avgTime,
                  icon: <Clock size={18} />,
                  color:
                    "bg-blue-50 text-blue-500 dark:bg-blue-900/40 dark:text-blue-400",
                },
                {
                  label: "Response Rate",
                  value: d.responseMetrics.rate,
                ...
                {
                  label: "Rating Impact",
                  value: d.responseMetrics.ratingImpact,
                ...
                <div
                  key={m.label}
                  className="flex flex-col items-center text-center p-4 bg-gray-50 dark:bg-slate-700/50 rounded-xl"
                >
                  <div
                    className={`w-10 h-10 grid place-items-center rounded-xl ${m.color} mb-3`}
                  >
                    {m.icon}
                  </div>
            ...
            <div className="mt-5 pt-4 border-t border-gray-100 dark:border-slate-700">
              <p className="text-[13px] text-gray-500 dark:text-gray-400 m-0 leading-relaxed">
                ...
                Responding within 2 hours correlates with
                <span className="font-semibold text-blue-600 dark:text-blue-400">
                  {" "}
                  {d.responseMetrics.ratingImpact}
                </span>{" "}
                star improvement in follow-up ratings.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-5">
            ...
            <div className="flex gap-1">
              <div className="flex flex-col gap-1 shrink-0 mr-1">
                {["M", "T", "W", "T", "F", "S", "S"].map((day, i) => (
                  <span
                    key={i}
                    className="text-[10px] text-gray-400 h-3.5 flex items-center justify-end w-4"
                  >
                    {day}
                  </span>
                ))}
            ...
            <div className="mt-4 pt-3 border-t border-gray-100 dark:border-slate-700 flex items-center justify-between">
              ...
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                Saturdays — avg{" "}
                {Math.round(
                  d.heatmapWeeks.reduce((s, w) => s + w[5], 0) /
                    d.heatmapWeeks.length,
                )}{" "}
                reviews
              </span>
            </div>
        ...
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-5">
            ...
            <div>
              ...
              <p className="m-0 text-[11px] text-gray-400 dark:text-gray-500">
                Actionable insights based on review analysis
              </p>
            </div>
          </div>
          <div className="flex flex-col gap-3">
            {d.aiActions.map((action, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`flex items-start gap-3.5 p-4 rounded-xl border ${styles.border} ${styles.bg}`}
                >
                  <div
                    className={`w-8 h-8 grid place-items-center rounded-lg bg-white dark:bg-slate-800 shrink-0 ${styles.iconColor}`}
                  >
                    {styles.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${styles.labelBg}`}
                      >
                        {styles.label}
                      </span>
