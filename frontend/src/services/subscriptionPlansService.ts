...
export const fetchSubscriptionUsage = (
  userId: string,
): Promise<SubscriptionUsageSummary> => {
  return apiClient.get<SubscriptionUsageSummary>(
    `/subscription-usage/${encodeURIComponent(userId)}`,
  );
};

export const fetchUserOrganizations = (): Promise<
