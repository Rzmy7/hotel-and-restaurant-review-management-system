...
class SourcesService {
  ...
  private mapBackendSourceToFrontend(s: any): Source {
    return {
      ...
      syncSchedule: (() => {
        ...
        const freq =
          typeof s.fetching_frequency === "number"
  ...
  async getSources(organizationId: string): Promise<Source[]> {
    const data = await apiClient.get<{ sources: any[] }>(
      `/api/source/organizations/${organizationId}/sources`,
    );
  ...
  async exportSyncLogs(organizationId: string): Promise<void> {
    const response = await apiClient.get<Blob>(
      `/api/source/organizations/${organizationId}/sync-logs/export`,
      {
        responseType: "blob",
      },
    );
  ...
  async clearSyncLogs(organizationId: string): Promise<void> {
    return apiClient.delete(
      `/api/source/organizations/${organizationId}/sync-logs/clear`,
    );
  }

  async addSource(organizationId: string, sourceData: any): Promise<Source> {
    const scheduleMapReverse: Record<string, number> = {
      daily: 1,
      three_days: 2,
      weekly: 3,
    };
