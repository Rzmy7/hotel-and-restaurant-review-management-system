...
const NotificationIcon: React.FC<NotificationIconProps> = ({ type }) => {
  ...
  return (
    <div
      className={`w-10 h-10 rounded-xl ${style.bg} ${style.color} grid place-items-center shrink-0 ring-1 ${style.ring} transition-all duration-300 group-hover:scale-110`}
    >
      {style.icon}
    </div>
  );
