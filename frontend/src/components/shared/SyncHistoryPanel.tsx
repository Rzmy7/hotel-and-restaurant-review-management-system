...
const getActivityIcon = (type?: string, status?: string) => {
  ...
  switch (type) {
    ...
    case "SYNC_STARTED":
      return <Rocket size={18} className="text-blue-500" />;
    case "SYNC_COMPLETED":
    ...
    case "SOURCE_ADDED":
      return <PlusCircle size={18} className="text-teal-500" />;
    case "SOURCE_REMOVED":
      return <Trash2 size={18} className="text-rose-400" />;
    default:
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                if (
                  confirm(
                    "Are you sure you want to clear the activity log? This will delete all history except the latest 5 entries.",
                  )
                ) {
                  onClear?.();
                }
        ...
        <div className="px-8 pb-8">
          <div className="flex bg-slate-100/50 dark:bg-slate-800/50 p-1 rounded-xl border border-slate-100 dark:border-slate-800">
            {[
              ...
              {
                label: "Syncs",
                filter: { type: "SYNC_STARTED,SYNC_COMPLETED" },
              },
              {
                label: "System",
                filter: {
                  type: "SOURCE_ADDED,SOURCE_REMOVED,SYNC_SCHEDULE_UPDATED",
                },
              },
            ].map((tab) => {
              const isActive =
                JSON.stringify(currentFilter) === JSON.stringify(tab.filter);
              return (
                <button
                  key={tab.label}
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-[#597FE6]/10 dark:bg-[#597FE6]/20 rounded-lg">
                  <RefreshIcon
                    size={14}
                    className="text-[#597FE6] animate-spin"
                  />
                </div>
        ...
        >
          ...
          {!isLoading && logs.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <h3 className="text-slate-900 dark:text-white font-black text-sm uppercase">
                No Activity Yet
              </h3>
