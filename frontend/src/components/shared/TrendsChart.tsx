...
const TrendsChart = ({ data }: TrendsChartProps) => {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm flex items-center justify-center h-[340px]">
        ...
        <div className="flex gap-6 mt-1">
          <div className="flex items-center gap-2 group cursor-help relative">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-gray-700 uppercase tracking-widest leading-none">
                Volume
              </span>
      ...
      <div className="relative h-[220px] mb-10 mt-4 px-10 group/chart">
        ...
        <div className="absolute bottom-[-32px] left-10 right-10 h-6 text-[9px] font-black text-gray-300 uppercase tracking-[0.05em] flex justify-between">
          {data.map((point, i) => (
            <span
              key={i}
              className="hover:text-gray-500 cursor-default transition-colors"
            >
              {point.label}
            </span>
          ))}
