...
const Sidebar: React.FC<SidebarProps> = ({ isExpanded, onToggle }) => {
  ...
  const sectionsWithLiveBadges: SidebarGroupData[] =
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                      org.id === currentOrg.id
                        ? "bg-brand text-white"
                        : "bg-gray-100 text-gray-500 group-hover/item:bg-brand/10 group-hover/item:text-brand dark:bg-slate-700 dark:text-gray-400 dark:group-hover/item:bg-blue-900/30 dark:group-hover/item:text-blue-400"
                    }`}
                  >
                    <Building2 size={14} />
                  </div>
...
const SidebarSection: React.FC<{
  ...
  return (
    <div className="mb-1">
      {showDivider && (
        <div className="h-px bg-gray-50 dark:bg-slate-800/80 my-2 mx-2" />
      )}
      {section.label && (
        <div
          className={`
