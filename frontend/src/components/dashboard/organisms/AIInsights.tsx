...
export const AIInsights: React.FC<AIInsightsProps> = ({ data }) => {
  ...
  return (
    <Card
      hoverEffect
      className="shadow-sm p-6 flex flex-col h-full group relative"
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 flex-1 z-10">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 px-1">
            ...
            <h4 className="m-0 text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">
              Key Strengths
            </h4>
          </div>
          <div className="space-y-2.5">
            {strengths.map((item) => (
              <div
                key={item.label}
                className="p-3.5 bg-blue-50/20 border border-blue-100/30 dark:bg-blue-900/10 dark:border-blue-900/30 rounded-xl hover:bg-blue-50/40 dark:hover:bg-blue-900/20 transition-colors group/item"
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[13px] font-black text-gray-800 dark:text-gray-200">
                    {item.label}
                  </span>
                  <ThumbsUp
                    size={12}
                    className="text-[#4e80ee] opacity-0 group-hover/item:opacity-100 transition-opacity"
                  />
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-[9px] font-black px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/40 text-[#4e80ee] dark:text-blue-400 rounded uppercase tracking-widest">
                    Impact: {item.impact}
                  </span>
                  <span className="text-[9px] font-bold text-gray-400 dark:text-slate-400 uppercase tracking-wider">
                    {item.freq} mention rate
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2.5">
            {issues.map((item) => (
              <div
                key={item.label}
                className="p-3.5 bg-rose-50/20 border border-rose-100/30 dark:bg-rose-900/10 dark:border-rose-900/30 rounded-xl hover:bg-rose-50/40 dark:hover:bg-rose-900/20 transition-colors group/item"
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[13px] font-black text-gray-800 dark:text-gray-200">
                    {item.label}
                  ...
                  <ThumbsDown
                    size={12}
                    className="text-rose-500 opacity-0 group-hover/item:opacity-100 transition-opacity"
                  />
                </div>
                <div className="flex items-center gap-3">
                  <span
      ...
      <div className="p-5 bg-blue-50/40 dark:bg-slate-800/80 border border-blue-100/60 dark:border-blue-900/30 rounded-2xl relative overflow-hidden group/highlight shadow-sm z-10">
        ...
        <div className="flex items-center gap-3 mb-3 relative z-10">
          ...
          <span className="text-[10px] font-black text-blue-600/80 dark:text-blue-400/80 uppercase tracking-[0.2em] italic">
            AI Highlight
          </span>
        </div>
