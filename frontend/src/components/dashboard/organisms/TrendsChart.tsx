...
export const TrendsChart: React.FC<TrendsChartProps> = ({ data }) => {
  ...
  if (!data || data.length === 0) {
    return (
      <Card
        hoverEffect
        className="shadow-sm p-6 flex items-center justify-center h-[340px]"
      >
  ...
  const maxSentiment =
    rawMaxSentiment > 100 ? Math.ceil(rawMaxSentiment / 10) * 10 : 100;

  // Generate points for volume
  const volumePoints = data.map((point, i) => {
    const x = data.length > 1 ? (i / (data.length - 1)) * width : width / 2;
    const y =
      height - (point.volume / maxVolume) * (height - padding * 2) - padding;
    return { x, y };
  });
  ...
  const sentimentPoints = data.map((point, i) => {
    ...
    const y =
      height -
      (point.sentiment / maxSentiment) * (height - padding * 2) -
      padding;
    return { x, y };
  });
  ...
  const getSmoothingPath = (pts: { x: number; y: number }[]) => {
    ...
    if (pts.length === 1)
      return `M ${pts[0].x} ${pts[0].y} L ${pts[0].x + 0.1} ${pts[0].y}`;
        ...
        <div className="flex gap-6 mt-1">
          <div className="flex items-center gap-2 group cursor-help relative">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest leading-none">
                Volume
              </span>
              <span className="text-[8px] font-bold text-gray-400 dark:text-slate-500 mt-1 uppercase">
                Total
              </span>
            </div>
          </div>
      ...
      <div className="relative h-[220px] mb-10 mt-4 px-10 group/chart">
        ...
        <div className="absolute bottom-[-32px] left-10 right-10 h-6 text-[9px] font-black text-gray-300 dark:text-slate-600 uppercase tracking-[0.05em] flex justify-between">
          {data.map((point, i) => (
            <span
              key={i}
              className="hover:text-gray-500 dark:hover:text-slate-400 cursor-default transition-colors"
            >
              {point.label}
            </span>
          ))}
            ...
            return (
              <g key={i} className="cursor-pointer group/marker">
                <circle
                  ...
                  fill="#4e80ee"
                  fillOpacity="0.1"
                  className="scale-0 group-hover/marker:scale-100 transition-transform duration-300"
                />
                <circle
                  cx={p.x}
                  cy={p.y}
