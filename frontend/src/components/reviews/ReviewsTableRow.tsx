...
const ReviewsTableRow = ({
  ...
  const getSentimentStyles = (sentiment: Review["sentiment"]) => {
    switch (sentiment) {
      case "Positive":
        return "text-emerald-500 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/40 border-emerald-100 dark:border-emerald-800";
      case "Negative":
        return "text-rose-500 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/40 border-rose-100 dark:border-rose-800";
      case "Neutral":
        return "text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-slate-800 border-gray-200 dark:border-slate-700";
    }
  };
    ...
    >
              ...
              <Star
                ...
                className={
                  i < review.rating
                    ...
                    : "fill-gray-100 dark:fill-slate-700 text-gray-200 dark:text-slate-600"
                }
              />
            ))}
          </div>
      ...
      <td className="px-6 py-5">
        <div className="flex gap-3">
          ...
          <div>
            <p className="text-[13px] font-black text-gray-900 dark:text-white mb-1 tracking-tight">
              {review.userName}
            </p>
            {review.heading && (
              <p className="text-[12px] font-black text-gray-800 dark:text-gray-200 mb-1 leading-tight line-clamp-1">
                {review.heading}
              </p>
            )}
            <p className="text-sm text-gray-600 dark:text-gray-300 line-clamp-2 leading-relaxed pr-8">
              {review.reviewText}
            </p>
          </div>
        </div>
      ...
      <td className="px-6 py-5">
        <div className="flex flex-col gap-2 items-start">
          <span
            className={`inline-flex px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider border ${getSentimentStyles(review.sentiment)}`}
          >
            {review.sentiment}
          </span>
          {review.categories && review.categories.length > 0 && (
            <div className="flex gap-1 flex-wrap relative group/cat z-10 w-fit">
              ...
              {review.categories.length > 1 && (
                <div
                  className={`absolute ${isLastRows ? "bottom-full mb-2" : "top-full mt-2"} left-0 hidden group-hover/cat:flex flex-col gap-1 w-max p-2 z-[60] bg-gray-900 border border-gray-800 rounded-xl shadow-2xl animate-in fade-in zoom-in-95 duration-200`}
                >
                  <div
                    className={`absolute ${isLastRows ? "-bottom-1.5" : "-top-1.5"} left-4 w-3 h-3 bg-gray-900 rotate-45 ${isLastRows ? "border-r border-b" : "border-l border-t"} border-gray-800`}
                  />
                  {review.categories.map((cat, i) => (
                    <span
                      key={i}
                      className="text-[11px] font-bold text-gray-200 relative z-10"
                    >
                      {cat}
                    </span>
                  ))}
