...
const ReviewDetailModal = ({
  ...
  const [tone, setTone] = useState<"professional" | "casual" | "standard">(
    "standard",
  );
  const [replyLength, setReplyLength] = useState<"short" | "standard">(
    "standard",
  );
  ...
  const handleGenerate = async (
    ...
    try {
      const generated = await reviewsService.generateReply(
        review,
        tone,
        length,
      );
  ...
  const handleMarkResolved = async () => {
    setIsResolving(true);
    try {
      await reviewsService.updateReviewStatus(
        organizationId,
        review.id,
        "Replied",
      );
  ...
  const getSentimentStyles = (sentiment: string) => {
    switch (sentiment) {
      case "Positive":
        return "bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-400 dark:border-emerald-800";
      case "Negative":
        return "bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/40 dark:text-rose-400 dark:border-rose-800";
      default:
                ...
                <Star
                  ...
                  className={
                    i < review.rating
                      ...
                      : "fill-gray-100 text-gray-200 dark:fill-slate-700 dark:text-slate-600"
                  }
                />
              ))}
            </div>
  ...
  const footer = (
    <div className="flex items-center justify-between w-full">
      <Button variant="ghost" onClick={onClose} className="text-[13px] px-6">
        Close View
      </Button>
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm relative">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    />
                    <h3 className="text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">
                      Customer Review
                    </h3>
                  </div>
                  {review.heading && (
                ...
                {review.photos && review.photos.length > 0 && (
                  <div>
                    <h3 className="text-[11px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest mb-4">
                      Attachments ({review.photos.length})
                    </h3>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-blue-100 dark:border-blue-900/50 overflow-hidden shadow-sm relative z-0">
                  ...
                  <div className="px-6 py-4 border-b border-blue-50 dark:border-blue-900/30 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Cpu
                        size={18}
                        className="text-[#4e80ee] dark:text-blue-400"
                      />
                      <h3 className="text-[12px] font-black text-[#4e80ee] dark:text-blue-400 uppercase tracking-widest">
                        AI Drafted Response
                      </h3>
                    </div>
                  ...
                  <div className="p-6">
                    <div className="min-h-[140px] w-full p-4 bg-white/80 dark:bg-slate-800/80 border border-blue-100/50 dark:border-blue-900/50 rounded-xl text-[14px] font-medium text-gray-700 dark:text-gray-200 focus-within:ring-4 focus-within:ring-blue-500/10 focus-within:border-blue-400 transition-all shadow-sm shadow-blue-50 dark:shadow-none relative overflow-hidden">
                      {isGenerating && (
                        <div className="absolute inset-0 bg-white/60 dark:bg-slate-800/60 backdrop-blur-[1px] flex items-center justify-center z-10">
                          <RefreshCw
                            size={24}
                            className="text-[#4e80ee] dark:text-blue-400 animate-spin"
                          />
                        </div>
                      )}
                      <textarea
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-4">
                      ...
                      <select
                        value={replyLength}
                        onChange={(e) =>
                          setReplyLength(e.target.value as "short" | "standard")
                        }
                        disabled={isGenerating || isSaving}
                        className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-[12px] font-semibold text-gray-700 dark:bg-slate-800 dark:border-slate-700 dark:text-gray-300"
                        title="Reply length"
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm relative overflow-hidden">
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      <span
                        className={`inline-flex px-3 py-1 rounded-lg text-[11px] font-black uppercase tracking-wider border ${getSentimentStyles(review.sentiment)}`}
                      >
                        {review.sentiment}
                      </span>
                    </div>
                    ...
                    {/* Categories */}
                    <div>
                      <span className="text-[10px] font-bold text-gray-400 dark:text-slate-400 uppercase tracking-widest block mb-2">
                        Topic Categories
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {review.categories?.length ? (
                          review.categories.map((cat, i) => (
                            <span
                              key={i}
                              className="px-2.5 py-1 rounded-md text-[11px] font-bold bg-gray-100 text-gray-600 border border-gray-200 dark:bg-slate-700 dark:text-gray-300 dark:border-slate-600"
                            >
                              {cat}
                            </span>
                          ))
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {review.keyPhrases?.length ? (
                          review.keyPhrases.map((phrase, i) => (
                            <span
                              key={i}
                              className="px-2.5 py-1 rounded-md text-[11px] font-bold bg-blue-50 text-[#4e80ee] border border-blue-100 dark:bg-blue-900/40 dark:text-blue-400 dark:border-blue-800"
                            >
                              "{phrase}"
                            </span>
                          ))
                    ...
                    <div>
                      ...
                      <p className="text-[13px] text-gray-600 bg-gray-50 border-gray-100 dark:text-gray-300 dark:bg-slate-700 dark:border-slate-600 leading-relaxed font-medium p-3 rounded-xl border">
                        {review.summary ||
                          "Summary not available for this review."}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-[12px]">
                      <span className="text-gray-500 font-bold dark:text-slate-400">
                        Review ID
                      </span>
                      <span className="text-gray-900 bg-gray-50 border-gray-100 dark:text-white dark:bg-slate-700 dark:border-slate-600 font-mono tracking-tighter px-2 py-0.5 rounded border">
                        {review.id}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-[12px]">
