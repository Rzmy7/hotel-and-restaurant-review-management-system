...
const ReviewsToolbar = () => {
  ...
  const { filters, setSearchQuery, setEmbeddingSearch, toggleFilter } =
    useReviewFilters();
  ...
  const isFilterActive = (type: string) => {
    switch (type) {
      ...
      case "Sentiment":
        return filters.sentiment.length > 0;
      case "Platform":
        return filters.source.length > 0;
      case "Category":
  ...
  const renderFilterMenu = (menu: string) => {
    ...
    let filterType:
      | keyof Omit<
          FilterState,
          "search" | "hasAiReply" | "useEmbeddingSearch" | "dateFrom" | "dateTo"
        >
      | "" = "";
    ...
    return (
      <div
        className={`absolute top-full left-0 mt-2 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl shadow-lg shadow-gray-200/50 dark:shadow-slate-900/50 z-50 p-2 min-w-[200px] ${hasSearch ? "min-w-[240px]" : ""}`}
      >
        {hasSearch && (
          <div className="mb-2">
            ...
            <br />
            {filterType &&
              (filters[filterType] as (string | number)[]).length > 0 && (
                <div className="flex flex-wrap gap-1 border-b border-gray-100 dark:border-slate-700 pb-2 mb-2">
                  {(filters[filterType] as (string | number)[]).map(
                    (val: string | number) => (
                      <button
                        key={val}
                        onClick={() =>
                          toggleFilter(
                            filterType as keyof Omit<
                              FilterState,
                              | "search"
                              | "hasAiReply"
                              | "useEmbeddingSearch"
        ...
        <div
          className={
            hasSearch
              ? "max-h-56 overflow-y-auto pr-1 space-y-0.5 custom-scrollbar"
              : "space-y-0.5"
          }
        >
          {menuOptions
            .filter((opt: string | number) =>
              hasSearch
                ? String(opt).toLowerCase().includes(menuSearch.toLowerCase())
                : true,
            )
            .map((opt: string | number) => {
              ...
              return (
                ...
                >
                  <span>
                    {menu === "Rating" ? `${opt} Stars` : String(opt)}
                  </span>
    ...
    <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
      ...
      <div className="w-full md:w-auto flex flex-col sm:flex-row sm:items-center gap-2">
        <div className="relative w-full md:w-96 group">
          <Search
            className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 group-focus-within:text-[#4e80ee] transition-colors"
            size={16}
          />
          <input
            type="text"
        ...
        {isContentSearchEnabled && (
          <button
            ...
            className={`inline-flex items-center gap-2 px-3 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-wider border transition-all ${
              filters.useEmbeddingSearch
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {Object.keys(options).map((menu) => (
          <div key={menu} className="relative">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[12px] font-black uppercase tracking-wider transition-all duration-300 border ${
                isFilterActive(menu) || activeMenu === menu
            ...
            >
              ...
              <ChevronDown
                size={14}
                className={`transition-transform ${activeMenu === menu ? "rotate-180" : ""}`}
              />
            </button>
        ...
        <div className="text-[11px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">
          Showing{" "}
          <span className="text-[#4e80ee] dark:text-blue-400 text-[13px]">
            {pagination.total}
          </span>{" "}
          results
        </div>
