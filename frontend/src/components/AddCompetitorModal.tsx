...
const AddCompetitorModal: React.FC<AddCompetitorModalProps> = ({
  ...
  const filteredCompetitors = competitors.filter(
    (competitor) => inferCompetitorDomain(competitor) === selectedDomain,
  );
        ...
        <div className="px-8 py-6 flex items-start justify-between border-b border-gray-100 dark:border-slate-700 flex-shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-400 dark:text-slate-400 mt-1">
              Available {selectedDomain === "Hotel" ? "hotel" : "restaurant"}{" "}
              competitors
            </p>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-8 py-4 custom-scrollbar">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <button
                onClick={handleRetry}
                className="flex items-center gap-1.5 text-blue-500 hover:text-blue-600 text-sm font-medium"
              >
            ...
            <div className="flex items-center justify-center py-12 text-sm text-gray-400 dark:text-slate-500">
              No available {selectedDomain === "Hotel" ? "hotel" : "restaurant"}{" "}
              competitors found.
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-100/60 dark:border-slate-700/60 sticky top-0 bg-white dark:bg-slate-800 z-10 flex w-full">
                  <th className="py-4 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest w-[40%]">
                    Competitor Name
                  </th>
                  <th className="py-4 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest w-[25%] flex-1">
                    Location
                  </th>
                  <th className="py-4 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest w-[20%] flex-1">
                    Avg Rating
                  </th>
                  <th className="py-4 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest w-[15%] text-center flex-1">
                    Action
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50/80 dark:divide-slate-700/80 flex flex-col w-full">
                {filteredCompetitors.map((competitor) => (
                  <tr
                    key={competitor.id}
                    className="hover:bg-gray-50/40 dark:hover:bg-slate-700/40 transition-colors flex w-full items-center"
                  >
                    <td className="py-[18px] w-[40%]">
                      <span className="font-semibold text-gray-800 dark:text-gray-200 text-[14px]">
                        {competitor.name}
                      </span>
                    </td>
                    <td className="py-[18px] flex-1">
                      <span className="text-gray-500 dark:text-slate-400 text-[14px]">
                        {competitor.location}
                      </span>
                    </td>
