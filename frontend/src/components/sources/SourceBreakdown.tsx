...
const MiniBar = ({
  ...
  <div className="flex h-2 rounded-full overflow-hidden w-full">
    <div
      style={{
        width: `${pctOf(positive, total)}%`,
        backgroundColor: "#10b981",
      }}
    />
    <div
      style={{ width: `${pctOf(neutral, total)}%`, backgroundColor: "#94a3b8" }}
    />
    <div
      style={{
        width: `${pctOf(negative, total)}%`,
...
const SourceBreakdown: React.FC<SourceBreakdownProps> = ({
  timeRange = "30d",
}) => {
      ...
      <div className="flex items-start justify-between mb-5 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="m-0 text-[11px] text-gray-400 dark:text-slate-400">
      ...
      {tab === "overview" && (
        <div className="flex flex-col gap-0 divide-y divide-gray-100 dark:divide-slate-700/50">
          ...
          <div className="grid grid-cols-[1fr_60px_60px_80px] gap-x-4 pb-2 mb-1">
            ...
            <span className="text-[11px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide text-center">
              Reviews
            </span>
            <span className="text-[11px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide text-center">
              Share
            </span>
            <span className="text-[11px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide text-right">
              Volume
            </span>
          </div>
          ...
          {sources.map((s) => (
            <div
              key={s.name}
              className="grid grid-cols-[1fr_60px_60px_80px] gap-x-4 items-center py-3"
            >
              {/* Source name */}
        ...
        <div className="flex flex-col gap-0 divide-y divide-gray-100 dark:divide-slate-700/50">
          ...
          <div className="flex items-center gap-4 pb-3 flex-wrap">
            {[
              ...
              <div
                key={label}
                className="flex items-center gap-1.5 text-[12px] text-gray-500 dark:text-slate-400"
              >
          ...
          <div className="grid grid-cols-[120px_1fr_56px_56px_56px] gap-x-3 py-2">
            ...
            <span
              className="text-[11px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide text-center"
              style={{ color: "#10b981" }}
            >
              Pos
            </span>
          ...
          {sources.map((s) => {
            ...
            return (
              <div
                key={s.name}
                className="grid grid-cols-[120px_1fr_56px_56px_56px] gap-x-3 items-center py-3.5"
              >
                {/* Source name */}
                <div className="flex items-center gap-2">
                  <span
