...
const SourceStats: React.FC<SourceStatsProps> = ({
  stats,
  isLoading,
  onFilterClick,
  activeFilter,
      ...
      {cards.map((card, index) => {
        ...
        return (
          <div
            ...
            className={`bg-white dark:bg-slate-800 p-5 rounded-xl border transition-all duration-300 relative overflow-hidden group ${
              isActive
                ? "border-blue-500 ring-2 ring-blue-500/10 shadow-md translate-y-[-2px]"
                : "border-gray-100 dark:border-slate-700 hover:shadow-lg hover:shadow-gray-200/40 dark:hover:shadow-slate-900/40 hover:-translate-y-0.5"
            } ${isClickable ? "cursor-pointer" : ""} ${card.border}`}
          >
            <div className="flex justify-between items-center mb-4">
              <div
                className={`w-10 h-10 flex items-center justify-center rounded-xl ${card.bg} ${card.color} transition-transform duration-300 group-hover:scale-110 shadow-sm border border-transparent`}
              >
                <card.icon size={20} />
              </div>
              {index === 1 && !isLoading && (
