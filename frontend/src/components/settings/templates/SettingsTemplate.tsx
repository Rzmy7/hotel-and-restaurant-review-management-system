...
export const SettingsTemplate: React.FC<SettingsTemplateProps> = ({
    ...
    return (
        <div className="min-h-full bg-gray-50 flex flex-col relative pb-24">
            ...
            {/* Main Content Area */}
            <main className="flex-1 w-full max-w-[1600px] mx-auto px-8 py-6 space-y-6">
                {children}
            </main>

            {/* Sticky Action Footer */}
            <div className="fixed bottom-0 right-0 left-0 lg:left-[280px] z-50 bg-white/80 backdrop-blur-md border-t border-gray-200 px-8 py-4 flex gap-4 justify-end shadow-[0_-4px_24px_rgba(0,0,0,0.02)] transition-all duration-300">
                <Button
                    variant="outline"
                    onClick={onCancel}
                    disabled={isSaving}
