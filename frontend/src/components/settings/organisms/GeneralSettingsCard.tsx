...
export const GeneralSettingsCard: React.FC<GeneralSettingsCardProps> = ({
  ...
  const [organizationPendingDelete, setOrganizationPendingDelete] =
    useState<UserOrganizationSummary | null>(null);
  ...
  const syncOrganizationCache = (items: UserOrganizationSummary[]) => {
    ...
    localStorage.setItem(
      "organization_ids",
      JSON.stringify(items.map((item) => item.organization_id)),
    );

    const currentOrganization = localStorage.getItem("current_organization");
    const hasCurrent =
      !!currentOrganization &&
      items.some((item) => item.organization_id === currentOrganization);
  ...
  return (
    <div className="flex flex-col">
      <FormField
        ...
        description="Manage organizations you own"
      >
      ...
      {darkModeAllowed && (
        <FormField
          ...
          orientation="horizontal"
          description="Select your preferred UI appearance"
        >
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-lg text-[13px] font-bold transition-all duration-300 ${
                    isActive
