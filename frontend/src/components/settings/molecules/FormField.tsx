...
export const FormField: React.FC<FormFieldProps> = ({
    ...
    return (
        <div className="flex flex-col gap-2 py-4 border-b border-gray-100 last:border-b-0 group">
            <label className="text-[13px] font-black tracking-tight text-gray-700 uppercase group-hover:text-blue-600 transition-colors">{label}</label>
            <div className="w-full max-w-[400px] max-md:max-w-full">
                {children}
            </div>
            {description && <p className="text-xs text-gray-400 font-bold uppercase tracking-wider">{description}</p>}
        </div>
    );
};
