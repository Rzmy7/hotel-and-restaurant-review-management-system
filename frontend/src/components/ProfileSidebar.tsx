...
const ProfileSidebar: React.FC<ProfileSidebarProps> = ({ profile, onPhotoChange }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-[12px] p-8 shadow-[0_1px_3px_rgba(0,0,0,0.1)] border border-gray-200 dark:border-slate-700 w-full box-border">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 text-center mb-6">{profile.jobTitle}</p>
            ...
            <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-gray-600 dark:text-slate-300 truncate">{profile.email}</span>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="text-lg w-6 flex justify-center">📍</div>
                    <span className="text-sm text-gray-600 dark:text-slate-300">{profile.location}</span>
                </div>

                <div className="flex items-center gap-3">
                    <div className="text-lg w-6 flex justify-center">📅</div>
                    <span className="text-sm text-gray-600 dark:text-slate-300">Joined {profile.joinedDate}</span>
                </div>
            </div>
        </div>
    );
