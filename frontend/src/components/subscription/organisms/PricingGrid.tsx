...
export const PricingGrid: React.FC = () => {
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    try {
      const { updateTenantPlan } =
        await import("../../../services/subscriptionPlansService");
  ...
  const cardPlans = useMemo(
    () =>
      plans.map((plan, index) => ({
        ...
        buttonText: isUpdating
          ? "Updating..."
          : toButtonText(plan, currentPlanId),
  ...
  if (isLoading) {
    return (
      <div className="text-center text-gray-500 dark:text-slate-400 font-medium">
  ...
  return (
    <div className="flex flex-col items-center">
      {successMessage && (
        <div className="mb-4 text-green-500 font-medium animate-in fade-in">
          {successMessage}
        </div>
      )}
