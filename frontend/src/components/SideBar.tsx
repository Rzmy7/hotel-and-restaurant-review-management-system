...
const Sidebar: React.FC<SidebarProps> = ({ isExpanded, onToggle }) => {
  ...
  // Click handler: if collapsed and clicking the active page icon, expand instead of navigating
  const handleItemClick = (path: string) => {
    if (!isExpanded && isActive(path)) {
  ...
  return (
    <nav
      className={`
        h-full bg-white border-r border-gray-200 flex flex-col font-sans text-gray-600
    ...
    >
      ...
      >
        ...
        <button
          ...
          className="w-9 h-9 bg-blue-500 text-white rounded-lg flex items-center justify-center font-bold text-sm shrink-0 cursor-pointer border-none hover:bg-blue-600 transition-colors duration-200"
        >
          HR
        </button>

        {/* Brand text — fades + collapses horizontally */}
        <div
          className={`
          ...
          `}
        >
          <span className="font-semibold text-gray-800 text-[15px] whitespace-nowrap">ReviewHub</span>
          <span className="text-xs text-gray-400 whitespace-nowrap">Grand Hotel NYC</span>
        </div>
      </div>

      {/* 2. SCROLLABLE MENU — takes remaining space minus the pinned footer */}
      ...
      {/* 3. FOOTER — pinned to bottom, always visible */}
      <div className="border-t border-gray-100 py-2 shrink-0">
        ...
        {/* Version tag — only shown when expanded */}
        <div
          className={`
...
const SectionLabel = ({ text, isExpanded }: { text: string; isExpanded: boolean }) => (
  <div
    className={`
      ...
      ${isExpanded ? 'opacity-100 max-h-8 mt-4 mb-1' : 'opacity-0 max-h-0 mt-0 mb-0'}
    `}
  >
    {text}
  </div>
  ...
  return (
    <div className="relative group px-2 mb-0.5">
      ...
      >
        ...
        {/* Text label — slides & fades in/out */}
        <span
          className={`
        ...
        {/* Badge — only shown when expanded, positioned inline */}
        {badge && isExpanded && (
        ...
        {/* Badge dot — visible when collapsed (notification hint) */}
        {badge && !isExpanded && (
      ...
      {!isExpanded && (
        ...
        >
          {text}
          {badge && (
            <span className="ml-1.5 bg-blue-400 text-white text-[10px] px-1 py-0.5 rounded-full">
              {badge}
            </span>
          )}
          {/* Arrow */}
